...
def front_label(l, seg, shape): # 차량 detection 용도
    ...
    for l_id in l_ids:
        ...
        if x_line[0] in ['6', '7', '8', '9', '10', '12', '13', '999']: # remove 12,13 for rider
            if len(l_box) > 0:
...
def easy_label(l, seg): # 차량 detection 용도
    ...
    for x_line, x_seg in zip(l, seg):
        ...
        if x_line[0] in [6, 12 ,13]:
            ...
            min_box_size = 0.01
        if x_line[0] in [6, 12 ,13]:
            is_long_box = h_temp > w_temp*5 or w_temp > h_temp*3
        else:
            is_long_box = h_temp > w_temp*1.5 or w_temp > h_temp*4.5
...
def fix_segment(segments):
    #fix label out of bounds
    seg_fix = []
    for segment in segments:
        new_seg = np.zeros_like(segment)
        for seg_i, (seg_x, seg_y) in enumerate(segment):
            new_seg[seg_i][0] = round(float(max(min(seg_x,1),0)),6)
            new_seg[seg_i][1] = round(float(max(min(seg_y,1),0)),6)

        seg_fix.append(new_seg)
        
    return seg_fix
    
def double_belt_seg(l, segments):
    chunk = 0.15
    
    l_fix = []
    new_segment = []
    for x_line, seg in zip(l, segments):
        min_seg_y = min(seg[:, 1])
        max_seg_y = max(seg[:, 1])

        if max_seg_y - min_seg_y <= chunk:
            chunk_loop = 1
            chunk_stride = chunk
        else:
            chunk_loop = math.floor((max_seg_y - min_seg_y) / chunk)
            chunk_stride = chunk - (((max_seg_y - min_seg_y) % chunk) / (chunk_loop-1))

        chunk_start_y = min_seg_y

        min_x = 1
        max_x = 0
        for chunk_loop_idx in range(chunk_loop):
            new_segline = []
            for seg_x, seg_y in seg:
                if chunk_start_y <= seg_y <= chunk_start_y + chunk:
                    new_segline.append([seg_x, seg_y])
                    if seg_x < min_x:
                        min_x = seg_x
                    if max_x < seg_x:
                        max_x = seg_x
            ...
            line_fixed = [x_line[0], 
                            min((min_x + max_x)/2, 1-chunk),
                            min(chunk_start_y+chunk/2, 1-chunk), 
                            chunk, 
                            chunk]
            if len(new_segline) > 0:
                l_fix.append(line_fixed)
                new_segment.append(np.array(new_segline))
            chunk_start_y = chunk_start_y + chunk_stride

    l = l_fix
    return l, new_segment




def double_belt_by_chunk(l, is_flip=False):
    #fix label out of bounds
    l_fix = []
    for x_line in l:
        line_fixed_temp = [(round(float(x_item),6)) for x_item in x_line[1:]]
        ...
        x_fixed = round(float((x1_temp+x2_temp)/2),6)
        y_fixed = round(float((y1_temp+y2_temp)/2),6)
        w_fixed = round(float(x2_temp-x1_temp),6)
        h_fixed = round(float(y2_temp-y1_temp),6)


        chunk = 0.15
        num_of_chunk = math.floor(round(float(y2_temp-y1_temp),6) / chunk) + 2

        lines_fixed = []
        for line_idx in range(num_of_chunk):
            if num_of_chunk > 1:
                chunk_stride_x = (x2_temp - x1_temp - chunk) / (num_of_chunk-1)
                chunk_stride_y = (y2_temp - y1_temp - chunk*0.8) / (num_of_chunk-1)
            else:
                chunk_stride_x = 0
                chunk_stride_y = 0
            if is_flip:
                chunk_x = x2_temp - chunk*0.5 - chunk_stride_x*line_idx
                chunk_y = y2_temp - chunk*0.3 - chunk_stride_y*line_idx
            else:
                if h_fixed > w_fixed*4:
                    chunk_x = (x1_temp + chunk*0.5 + chunk_stride_x*line_idx)*0.5 + ((x1_temp+x2_temp)/2)*0.5
                    chunk_y = y2_temp - chunk*0.3 - chunk_stride_y*line_idx
                else:
                    chunk_x = x1_temp + chunk*0.5 + chunk_stride_x*line_idx
                    chunk_y = y2_temp - chunk*0.3 - chunk_stride_y*line_idx

            line_fixed = [x_line[0], str(round(float(chunk_x),6)), 
                            str(round(float(chunk_y),6)), 
                            str(chunk), 
                            str(chunk)]
            lines_fixed.append(line_fixed)

        for line_fixed in lines_fixed:
            l_fix.append(line_fixed)
...
def double_belt(l, is_flip=False):
    #fix label out of bounds
    l_fix = []
    for x_line in l:
        line_fixed_temp = [(round(float(x_item),6)) for x_item in x_line[1:]]
        ...
        if is_flip:                    
            line0_fixed = [x_line[0], str(round(float((x1_temp*0.8+x_temp*0.2)),6)), 
                            str(round(float((y1_temp*0.9+y_temp*0.1)),6)), 
                            str(round(float(w_temp*0.4),6)), 
                            str(round(float(h_temp*0.4),6))]

            line1_fixed = [x_line[0], str(round(float((x1_temp*0.6+x_temp*0.4)),6)), 
                            str(round(float((y1_temp*0.67+y_temp*0.33)),6)), 
                            str(round(float(w_temp*0.4),6)), 
                            str(round(float(h_temp*0.4),6))]

            line2_fixed = [x_line[0], str(round(float((x1_temp*0.4+x_temp*0.6)),6)), 
                            str(round(float((y1_temp*0.43+y_temp*0.57)),6)), 
                            str(round(float(w_temp*0.4),6)), 
                            str(round(float(h_temp*0.4),6))]

            line3_fixed = [x_line[0], str(round(float((x1_temp*0.2+x_temp*0.8)),6)), 
                            str(round(float((y1_temp*0.2+y_temp*0.8)),6)), 
                            str(round(float(w_temp*0.4),6)), 
                            str(round(float(h_temp*0.4),6))]
                            
            line4_fixed = [x_line[0], str(round(float(x_temp),6)), 
                            str(round(float(y_temp),6)), 
                            str(round(float(w_temp*0.4),6)), 
                            str(round(float(h_temp*0.4),6))]

            line5_fixed = [x_line[0], str(round(float((x_temp*0.8+x2_temp*0.2)),6)), 
                            str(round(float((y_temp*0.8+y2_temp*0.2)),6)), 
                            str(round(float(w_temp*0.4),6)), 
                            str(round(float(h_temp*0.4),6))]

            line6_fixed = [x_line[0], str(round(float((x_temp*0.6+x2_temp*0.4)),6)), 
                            str(round(float((y_temp*0.6+y2_temp*0.4)),6)), 
                            str(round(float(w_temp*0.4),6)), 
                            str(round(float(h_temp*0.4),6))]

            line7_fixed = [x_line[0], str(round(float((x_temp*0.4+x2_temp*0.6)),6)), 
                            str(round(float((y_temp*0.4+y2_temp*0.6)),6)), 
                            str(round(float(w_temp*0.4),6)), 
                            str(round(float(h_temp*0.4),6))]
        else:
            if h_fixed > w_fixed*4:
                line0_fixed = [x_line[0], str(round(float((x_temp*0.6+x2_temp*0.4)),6)), 
                                str(round(float((y1_temp*0.9+y_temp*0.1)),6)), 
                                str(round(float(w_temp*0.4),6)), 
                                str(round(float(h_temp*0.4),6))]

                line1_fixed = [x_line[0], str(round(float((x_temp*0.7+x2_temp*0.3)),6)), 
                                str(round(float((y1_temp*0.67+y_temp*0.33)),6)), 
                                str(round(float(w_temp*0.4),6)), 
                                str(round(float(h_temp*0.4),6))]

                line2_fixed = [x_line[0], str(round(float((x_temp*0.8+x2_temp*0.2)),6)), 
                                str(round(float((y1_temp*0.43+y_temp*0.57)),6)), 
                                str(round(float(w_temp*0.4),6)), 
                                str(round(float(h_temp*0.4),6))]

                line3_fixed = [x_line[0], str(round(float((x_temp*0.9+x2_temp*0.1)),6)), 
                                str(round(float((y1_temp*0.2+y_temp*0.8)),6)), 
                                str(round(float(w_temp*0.4),6)), 
                                str(round(float(h_temp*0.4),6))]
                ...
                line5_fixed = [x_line[0], str(round(float((x1_temp*0.1+x_temp*0.9)),6)), 
                                str(round(float((y_temp*0.8+y2_temp*0.2)),6)), 
                                str(round(float(w_temp*0.4),6)), 
                                str(round(float(h_temp*0.4),6))]

                line6_fixed = [x_line[0], str(round(float((x1_temp*0.2+x_temp*0.8)),6)), 
                                str(round(float((y_temp*0.6+y2_temp*0.4)),6)), 
                                str(round(float(w_temp*0.4),6)), 
                                str(round(float(h_temp*0.4),6))]

                line7_fixed = [x_line[0], str(round(float((x1_temp*0.3+x_temp*0.7)),6)), 
                                str(round(float((y_temp*0.4+y2_temp*0.6)),6)), 
                                str(round(float(w_temp*0.4),6)), 
                                str(round(float(h_temp*0.4),6))]
            else:                    
                line0_fixed = [x_line[0], str(round(float((x_temp*0.2+x2_temp*0.8)),6)), 
                                str(round(float((y1_temp*0.9+y_temp*0.1)),6)), 
                                str(round(float(w_temp*0.4),6)), 
                                str(round(float(h_temp*0.4),6))]
                ...
                line2_fixed = [x_line[0], str(round(float((x_temp*0.6+x2_temp*0.4)),6)), 
                                str(round(float((y1_temp*0.43+y_temp*0.57)),6)), 
                                str(round(float(w_temp*0.4),6)), 
                                str(round(float(h_temp*0.4),6))]

                line3_fixed = [x_line[0], str(round(float((x_temp*0.8+x2_temp*0.2)),6)), 
                                str(round(float((y1_temp*0.2+y_temp*0.8)),6)), 
                                str(round(float(w_temp*0.4),6)), 
                                str(round(float(h_temp*0.4),6))]
                ...
                line5_fixed = [x_line[0], str(round(float((x1_temp*0.2+x_temp*0.8)),6)), 
                                str(round(float((y_temp*0.8+y2_temp*0.2)),6)), 
                                str(round(float(w_temp*0.4),6)), 
                                str(round(float(h_temp*0.4),6))]

                line6_fixed = [x_line[0], str(round(float((x1_temp*0.4+x_temp*0.6)),6)), 
                                str(round(float((y_temp*0.6+y2_temp*0.4)),6)), 
                                str(round(float(w_temp*0.4),6)), 
                                str(round(float(h_temp*0.4),6))]

                line7_fixed = [x_line[0], str(round(float((x1_temp*0.6+x_temp*0.4)),6)), 
                                str(round(float((y_temp*0.4+y2_temp*0.6)),6)), 
                                str(round(float(w_temp*0.4),6)), 
                                str(round(float(h_temp*0.4),6))]

        l_fix.append(line0_fixed)
        l_fix.append(line1_fixed)
        l_fix.append(line2_fixed)
        l_fix.append(line3_fixed)
        l_fix.append(line4_fixed)
        l_fix.append(line5_fixed)
        l_fix.append(line6_fixed)
        l_fix.append(line7_fixed)
    l = l_fix
    l = np.unique(l, axis=0)
    return l
...
def natural_minmax_crop(image, targets, base_size, min_label_size_limit=None, max_label_size_limit=None):#16, 96
    ...
    target_sizes = (targets[:, 3] * targets[:, 4] * (height * width)) ** 0.5
    if len(target_sizes) > 0:
        if min_label_size_limit is not None:
            min_label_size = np.min(target_sizes)
            if 0 < min_label_size < min_label_size_limit:
                natural_min_scale = min_label_size_limit / min_label_size
            else:
        ...
        if max_label_size_limit is not None:
            ...
            if max_label_size_limit < max_label_size:
                natural_max_scale = max_label_size_limit / max_label_size
            else:
    ...
    if natural_scale is not None:
        ...
        if image.shape[0] > base_h and image.shape[1] > base_w:
            ...
            crop_x1 = max(int(random_crop_x - base_w/2), 0)
            crop_y1 = max(int(random_crop_y - base_h/2), 0)
            crop_x2 = min(int(crop_x1 + base_w), patch_width)
            crop_y2 = min(int(crop_y1 + base_h), patch_height)
            ...
            black_margin_y = int((max_base_size - patch_height)/2)
            base_image[black_margin_y:black_margin_y+patch_height, black_margin_x:black_margin_x+patch_width, :] = image
            fit_margin_x = int((max_base_size - base_w)/2)
            fit_margin_y = int((max_base_size - base_h)/2)
            image = base_image[fit_margin_y:fit_margin_y+base_h, fit_margin_x:fit_margin_x+base_w, :]
            new_targets = targets.copy()
            #new_targets[:, 1:4] = new_targets[:, 1:4] * natural_scale
            new_targets[:, 1] = ((new_targets[:, 1])*patch_width + black_margin_x - fit_margin_x) / image.shape[1]
            new_targets[:, 2] = ((new_targets[:, 2])*patch_height + black_margin_y - fit_margin_y) / image.shape[0]
            new_targets[:, 3] = new_targets[:, 3] * (patch_width/image.shape[1])
            new_targets[:, 4] = new_targets[:, 4] * (patch_height/image.shape[0])
    else:
        image = image
...
class DistributedWeightedSampler(torch.utils.data.distributed.DistributedSampler):
    ...
    def __iter__(self):
        ...
        #print(self.dataset.img_files[-2000:])
        #print(sampling_ratios[-2000:])
        #print(len(sampling_ratios))
...
def img2seg_paths(img_paths):
    ...
    for x in img_paths:
        ...
        if os.path.exists(seg_path):# and is_seg:
            ...
            seg_list.append("DUMMY0fkdskfoisdofj839urf0dcnf98du9fsjg")
            #print("No segments for "+seg_path)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, ratio_maintain=True, image_weights=False,
        ...
        if hyp is not None:
            if hyp.get('piecewise_augment', False) and hyp.get('piecewise_augment', False): 
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                if os.path.isfile(lb_file):
                    if os.path.basename(lb_file).startswith('g') and os.path.basename(lb_file).split('g')[-1].split('.txt')[0].isnumeric():
                        is_g_images = True
                    else:
                        is_g_images = False
                    nf += 1  # label found
                    with open(lb_file, 'r') as f:
                        ...
                        if load_seg:  # is segment
                            if any([len(x) > 8 for x in l]):  # is segment
                                ...
                                #    l, segments = double_belt_seg(l, segments)
                            else:
                                l = fix_label(l)
                                if right_belt_segment:
                                    #l = double_belt_by_chunk(l, is_flip=is_g_images)
                                    if is_g_images:   
                                        segments = [np.array([
                                            [float(x_line[1])-float(x_line[3])*0.0, float(x_line[2])-float(x_line[4])/2], 
                                            [float(x_line[1])-float(x_line[3])/2, float(x_line[2])-float(x_line[4])/2],  
                                            [float(x_line[1])-float(x_line[3])/2, float(x_line[2])-float(x_line[4])*0.0],
                                            [float(x_line[1])+float(x_line[3])*0.0, float(x_line[2])+float(x_line[4])/2],
                                            [float(x_line[1])+float(x_line[3])/2, float(x_line[2])+float(x_line[4])/2],
                                            [float(x_line[1])+float(x_line[3])/2, float(x_line[2])+float(x_line[4])*0.0],
                                            ]) for x_line in l]
    ...
    def __getitem__(self, index):
        ...
        if hyp is not None and hyp.get('randomwave', 0) > 0:
            nL = len(labels)  # number of labels
            if nL:
                for label in labels:
                    if random.random() < hyp['randomwave']:
                        ore_x1 = int(label[1])
                        ore_y1 = int(label[2])
                        ore_x2 = int(label[3])
                        ore_y2 = int(label[4])

                        image_patch = img[ore_y1:ore_y2, ore_x1:ore_x2, :].copy()
                        image_patch = random_wave(image_patch)
                        
                        img[ore_y1:ore_y2, ore_x1:ore_x2, :] = image_patch

        if hyp is not None and hyp.get('randomnoise', 0) > 0:
            if random.random() < hyp['randomnoise']:
                noise_max = random.randint(6, 24)
                img-int(noise_max/2)+np.random.rand(img.shape[0], img.shape[1], 3)*noise_max
        ...
        # remove full label
        '''
        valid_labels = torch.zeros_like(labels_out)
        valid_segments = torch.zeros_like(segments_out)
        valid_num = 0
        valid_idx = 0
        for label_out, segment_out in zip(labels_out, segments_out):
            label_size = (label_out[4] * label_out[5] * segment_out.shape[0] * segment_out.shape[1]).detach().cpu().numpy()
        ...
        labels_out = valid_labels[:valid_num]
        segments_out = valid_segments[:valid_num]
        '''
        # remove full label
...
def load_image_and_label(self, index, ratio_maintain=True, hyp=None):
...
def random_perspective(img, targets=(), segments=(), poses=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    if n:
        ...
        for i, segment in enumerate(segments):
            ...
            # clip
            new[i] = segment2box(xy_seg, width, height)
