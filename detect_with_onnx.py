...
def detect(save_img=False):
    ...
    person_det_model = onnx.load('fieheye_v0.onnx')
    onnx.checker.check_model(person_det_model)
    person_det_ort_session = onnxruntime.InferenceSession('weights/fieheye_v0.pt')
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        #preprocessed_data_before_crop = (cv2.cvtColor(img.transpose(1, 2, 0), cv2.COLOR_BGR2RGB) / 255).astype(np.float32)
        preprocessed_data_before_crop = (img.transpose(1, 2, 0) / 255).astype(np.float32) # 이미 rgb2bgr 되어있음
        preprocessed_data_before_crop = np.ascontiguousarray(preprocessed_data_before_crop.transpose(2, 0, 1))
        preprocessed_data_before_crop = np.expand_dims(preprocessed_data_before_crop, axis=0)
        person_det_ort_inputs = {person_det_ort_session.get_inputs()[0].name: preprocessed_data_before_crop}
        person_det_results = person_det_ort_session.run(None, person_det_ort_inputs)
        person_boxes = person_det_results[0]
        print(person_boxes)
                    ...
                    for *xyxy, conf, cls in reversed(det[:, :6]):
                        ...
                        if save_img or view_img:  # Add bbox to image
                            ...
                            for person_box in person_boxes:
                                if person_box[4] < 320:
                                    person_box[1] = person_box[1] * (im0.shape[0] / img.shape[2])
                                    person_box[2] = person_box[2] * (im0.shape[1] / img.shape[3])
                                    person_box[3] = person_box[3] * (im0.shape[0] / img.shape[2])
                                    person_box[4] = person_box[4] * (im0.shape[1] / img.shape[3])
                                    label = f'person {person_box[-1]:.2f}'
                                    plot_one_box(person_box[1:5], im0, label=label, color=colors[1], line_thickness=3)

                                    with open(txt_path + '.txt', 'a') as f:
                                        line_x = (person_box[1]+person_box[3])/2/im0.shape[1]
                                        line_y = (person_box[2]+person_box[4])/2/im0.shape[0]
                                        line_w = (person_box[3]-person_box[1])/im0.shape[1]
                                        line_h = (person_box[4]-person_box[2])/im0.shape[0]
                                        line = (1, line_x, line_y, line_w, line_h)
                                        f.write(('%g ' * len(line)).rstrip() % line + '\n')
