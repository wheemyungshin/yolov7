...
def detect(save_img=False):    
    results, seg_results, view_img, save_txt, imgsz = opt.results, opt.seg_results, opt.view_img, opt.save_txt, opt.img_size
    ...
    seg_source = os.path.join(seg_results, 'mask_npy')
    

    # 신호등 classification 모델 불러오기
    model_cls = mobilenet_v2(pretrained=True)
    model_cls.fc = nn.Linear(2048, 5)
    model_cls.load_state_dict(torch.load('weights/mobilenetv2_five_0061.pt'))
    ...
    road_indices = [13, 14, 15, 16, 17]
    seg_colors = [[100, 10, 10], [100, 255, 255], [255, 255, 100], [225, 225, 225], [255, 100, 255]]
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            if os.path.exists(os.path.join(seg_source, p.name.split('/')[-1].split('_clean')[0]+'.npy')):
                image_masks = np.load(os.path.join(seg_source, p.name.split('/')[-1].split('_clean')[0]+'.npy'))
                ...
                with open(os.path.join(det_source, p.name.split('/')[-1].split('_clean')[0]+'.txt'), 'r') as f:
                    ...
                    for xyxy_line_idx, xyxy_line in enumerate(xyxy_lines):
                        if len(xyxy_line.strip().split(' ')) == 5:
                            ...
                            conf = None
                        elif len(xyxy_line.strip().split(' ')) == 6:
                            c, x, y, w, h, conf = xyxy_line.strip().split(' ')
                        else:
                            c, x, y, w, h, conf = None, None, None, None, None, None
                        if c is not None:
                            cls = int(c)
                            if conf is not None:
                                if float(conf) < opt.conf_thres:
                                    continue
                                '''
                                if cls == 10:
                                    if float(conf) < 0.3:
                                        continue
                                elif float(conf) < opt.conf_thres:
                                    continue
                                elif cls == 6 and float(conf) < 0.8:
                                    continue
                                '''
                            if cls in class_name_dict:
                                ...
                                #if cls !=2 and (float(h)*im0.shape[0])*(float(w)*im0.shape[1]) < 800:
                                #    continue
                    ...
                    if save_img:
                        if vid_path != save_path.split('_clean')[0].split('_'+save_path.split('_clean')[0].split('_')[-1])[0]:  # new video
                            ...
                            if vid_cap:  # video
                                fps = 60#vid_cap.get(cv2.CAP_PROP_FPS)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seg-results', type=str, default='runs/detect')
