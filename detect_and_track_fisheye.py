...
def detect(save_img=False):
    ...
    prev_identities = []
    new_identity_warning_dict = {}
    new_identity_warning_life = 60
    new_identity_warning_timepoint = 30
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            new_det = []
            overlap_chain = []
            for i1, det1 in enumerate(det):
                for i2, det2 in enumerate(det):
                    if i1 != i2:
                        if check_boxes_overlap(det1[:4], det2[:4]):
                            if len(overlap_chain) > 0:
                                for overlap_chain_i, overlap_chain_ in enumerate(overlap_chain):
                                    if i1 in overlap_chain_ and i2 not in overlap_chain_:
                                        overlap_chain[overlap_chain_i].append(i2)
                                    elif i1 not in overlap_chain_ and i2 in overlap_chain_:
                                        overlap_chain[overlap_chain_i].append(i1)
                                    elif i1 not in overlap_chain_ and i2 not in overlap_chain_:
                                        overlap_chain.append([i1, i2])
                            else:
                                overlap_chain.append([i1, i2])
            
            print("det bef: ", det)
            print("overlap_chain: ", overlap_chain)
            for overlap_chain_ in overlap_chain:
                new_det_ = det[0].clone()
                new_det_[0] = min(det[overlap_chain_, 0])
                new_det_[1] = min(det[overlap_chain_, 1])
                new_det_[2] = max(det[overlap_chain_, 2])
                new_det_[3] = max(det[overlap_chain_, 3])
                new_det.append(new_det_.unsqueeze(0))
            for det_i, det_ in enumerate(det):
                is_merged_det = False
                for overlap_chain_ in overlap_chain:
                    if det_i in overlap_chain_:
                        is_merged_det = True
                if not is_merged_det:
                    new_det.append(det_.unsqueeze(0))
            det = torch.cat(new_det, dim=0)
            print("det aft: ", det)
            '''
            ...
            if dataset.mode != 'image' and frame % frame_ratio != 0:
                ...
                if len(det):
                    ...
                    new_identity_warning = False
                    for identity in identities:
                        if identity not in prev_identities:
                            new_identity_warning = True
                            new_identity_warning_dict[identity] = new_identity_warning_life
                    print("identities: ", identities)

                    for box_id, (*xyxy, cls, identity) in enumerate(reversed(tracked_dets[:, :6])):
                        if identity in new_identity_warning_dict and 0 < new_identity_warning_dict[identity] < new_identity_warning_timepoint:
                            box_marin = 15
                            plot_one_box([int(xyxy[0]-box_marin), int(xyxy[1]-box_marin), int(xyxy[2]+box_marin), int(xyxy[3]+box_marin)], im0, label="WARNING", color=[0, 0, 255], line_thickness=3)
                    ...
                    for warning_identity in list(new_identity_warning_dict.keys()):
                        new_identity_warning_dict[warning_identity] = new_identity_warning_dict[warning_identity] - 1
                        if new_identity_warning_dict[warning_identity] < 0:
                            new_identity_warning_dict[warning_identity] = 0
                            #new_identity_warning_dict.pop(warning_identity, None)
