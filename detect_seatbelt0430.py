...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        print(img.shape)
        if opt.square:
            mini_center_move = int(opt.center_move*(square_size/im0s[0].shape[0]))
            square_crop_margin = int((img.shape[2] - square_size) / 2) + mini_center_move
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if opt.square:
                square_crop_margin = int((im0.shape[1] - im0.shape[0]) / 2) + opt.center_move
                im0 = im0[:, square_crop_margin : square_crop_margin+im0.shape[0], :]
            clean_im0 = im0.copy()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--center-move', default=0, type=int, help='move center by pixel')
