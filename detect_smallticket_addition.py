...
def check_box_in_box(box1, box2, margin=0):
    couples = []
    if box2[0] - margin <= box1[0] <= box2[2] + margin and \
        box2[1] - margin <= box1[1] <= box2[3] + margin and \
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if opt.all_quadrant_at_once:
            ...
            full_img = cv2.resize(full_img, dsize=(640, 384))
            full_img = np.transpose(full_img, (2,0,1))
            full_img = torch.from_numpy(full_img).to(device)
            full_img = full_img.half() if half else full_img.float()  # uint8 to fp16/32
            full_img /= 255.0  # 0 - 255 to 0.0 - 1.0
            if full_img.ndimension() == 3:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det[:, :6]):
                    if cls == 2 and (xyxy[2]-xyxy[0])*1.2 < (xyxy[3]-xyxy[1]):
                        continue

                    traffic_light_in_other_traffic_light = False
                    if cls == 2:
                        for xyxy_ddds in det[:, :6]:
                            xyxy_ddd = xyxy_ddds[:4]
                            cls_ddd = xyxy_ddds[-1]
                            if xyxy[0] == xyxy_ddd[0] and xyxy[1] == xyxy_ddd[1] and xyxy[2] == xyxy_ddd[2] and xyxy[3] == xyxy_ddd[3]:
                                continue 
                            if cls_ddd == 2 and check_box_in_box([xyxy[0], xyxy[1], xyxy[2], xyxy[3]], [xyxy_ddd[0], xyxy_ddd[1], xyxy_ddd[2], xyxy_ddd[3]], 16):
                                traffic_light_in_other_traffic_light = True

                    if traffic_light_in_other_traffic_light:
                        continue                                   
