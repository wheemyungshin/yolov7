...
def distance_calibration(height, calibration_dict=None, y_bottom_pix=None, distance=None, stride=10, max_calib_length=50, inlier_range=2):
    if calibration_dict is None:
        calibration_dict = {}
        for s in range(0, height, stride):
            calibration_dict[s] = []
        return calibration_dict, None
    else:
        assert (distance is not None) and (y_bottom_pix is not None)
        edited_distance = distance
        for s in range(0, height, stride):
            if s <= y_bottom_pix < s+stride:
                if len(calibration_dict[s]) < max_calib_length:
                    calibration_dict[s].append(distance)
                else:

                    mean_distance = sum(calibration_dict[s]) / len(calibration_dict[s])
                    if mean_distance-inlier_range < distance < mean_distance+inlier_range:
                        calibration_dict[s][:-1] = calibration_dict[s][1:]
                        calibration_dict[s][-1] = distance
                    else:
                        edited_distance = round(mean_distance*0.9+distance*0.1, 1)
                        calibration_dict[s][:-1] = calibration_dict[s][1:]
                        calibration_dict[s][-1] = edited_distance
                break
        return calibration_dict, edited_distance
...
def detect(save_img=False):
    ...
    calibration_dict, _ = distance_calibration(1080)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if prev_det is not None:
            ...
            pred[0][tracking_valid_idx, 4] = pred[0][tracking_valid_idx, 4]+0.4
        ...
        pred_lane[0][:, 3] = pred_lane[0][:, 3] + crop_y1
        ...
        center_polygon = [
            ...
            int(im0.shape[1]*0.48), int(im0.shape[0]-1),
        ]
        ...
        lane_x_center = (left_xyxy[0]+left_xyxy[2]+right_xyxy[0]+right_xyxy[2])/4        
        left_dist = global_x_center-left_xyxy[0]
        right_dist = right_xyxy[2]-global_x_center

        if left_close > 0:
            if left_dist > right_dist:
                left_warning = warning_max
            else:
                left_warning = max(left_warning-1, 0)
        else:
            left_warning = max(left_warning-1, 0)

        if right_close > 0:
            if left_dist < right_dist:
                right_warning = warning_max
            else:
                right_warning = max(right_warning-1, 0)
        else:
            right_warning = max(right_warning-1, 0)

        if left_dist*4 < right_dist:
            left_close = close_max
        else:
            left_close = max(left_close-1, 0)
