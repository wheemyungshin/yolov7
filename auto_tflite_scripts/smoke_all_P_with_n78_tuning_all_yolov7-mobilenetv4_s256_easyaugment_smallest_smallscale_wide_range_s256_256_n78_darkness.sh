...
target="runs/train/smoke_all_P_with_n78_tuning_all_yolov7-mobilenetv4_s256_easyaugment_smallest_smallscale_wide_range_s256_256_n78_darkness/weights"
save_tag="modified_GN_"
for w in $target/*; do
        if [[ "$w" == *epoch*.pt ]]; then
            weight=${w##*/}
            echo "${weight%.*}"
