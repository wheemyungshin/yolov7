...
target="weights_cityscapes_ADAS_only_with_bicyclebustruck_easy_IDDv3_AW_vehicle_only_test_india_yolov7-mobilenet_easyaugment_smallest_min_lrtune_s384_384_3vcls"
for w in $target/*; do
        if [[ "$w" == *epoch*.pt ]]; then
            weight=${w##*/}
            echo "${weight%.*}"
