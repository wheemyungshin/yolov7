...
target="runs/train/smoke_all_P_n78_tuning_only13_yolov7-mobilenetv4_s256_easyaugment_smallest_smallscale_wide_range_lrtune7_3_from_all_wide_range_s256_256_n78_darkness2/weights"
save_tag="modified_GO_"
for w in $target/*; do
        if [[ "$w" == *epoch*.pt ]]; then
            weight=${w##*/}
            echo "${weight%.*}"
            ...
            python onnx_head_remover.py --type mobilenetv4 --model-name ${basename}_no_opt_128_128.onnx --dir ${target} --save-tag ${save_tag}
