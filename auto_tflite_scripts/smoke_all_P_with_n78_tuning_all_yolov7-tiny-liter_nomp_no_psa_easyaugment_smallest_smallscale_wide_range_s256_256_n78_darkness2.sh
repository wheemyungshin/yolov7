...
target="weights_smoke_all_P_with_n78_tuning_all_yolov7-tiny-liter_nomp_no_psa_easyaugment_smallest_smallscale_wide_range_s256_256_n78_darkness2"
save_tag="modified_GJ_"
for w in $target/*; do
        if [[ "$w" == *epoch*.pt ]]; then
            weight=${w##*/}
            echo "${weight%.*}"
