...
target="weights_smoke_all_P_n78_tuning_only13_yolov7-tiny-liter_nomp_easyaugment_smallest_smallscale_wide_range_lrtune7_3_from_EG_s256_256_n78_darkness"
save_tag="modified_GI_"
for w in $target/*; do
        if [[ "$w" == *epoch*.pt ]]; then
            weight=${w##*/}
            echo "${weight%.*}"
