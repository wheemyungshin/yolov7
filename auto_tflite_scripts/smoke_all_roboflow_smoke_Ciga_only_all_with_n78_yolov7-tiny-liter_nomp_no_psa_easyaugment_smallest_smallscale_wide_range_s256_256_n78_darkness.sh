...
target="weights_smoke_all_roboflow_smoke_Ciga_only_all_with_n78_yolov7-tiny-liter_nomp_no_psa_easyaugment_smallest_smallscale_wide_range_s256_256_n78_darkness"
for w in $target/*; do
        if [[ "$w" == *epoch*.pt ]]; then
            weight=${w##*/}
            echo "${weight%.*}"
