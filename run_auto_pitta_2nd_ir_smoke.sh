...
for number in ${numbers[@]}; do
        for subject in ${subjects[@]}; do
                if [ -d "${target}/${number}/${subject}" ]; then
                        python3 detect.py --weights weights/smoke_yolov7-tiny_s192_320.pt --conf 0.5 --iou-thres 0.1 --source ${target}/${number}/${subject} --name ${target##*/}_${number##*/}_${subject##*/} --save-json --save-txt --img-size 192 352 --device 0,1,2,3 --agnostic-nms --frame-ratio 30 --save-frame
                fi
        done
done