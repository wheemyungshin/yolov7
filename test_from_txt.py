...
def test(data,
    ...
    assert opt.predictions is not None
    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        out = os.path.join(opt.predictions, os.path.basename(paths[0]).replace('.jpg', '.txt'))
        pred_data = []
        if os.path.exists(out):
            with open(out, 'r') as f:
                ...
                for out_line in out_lines:
                    ...
                    x2 = (x + w/2)*width
                    y2 = (y + h/2)*height
                    pred_data.append([x1, y1, x2, y2, 1.0, int(out_line[0])])
        else:
            pred_data = []
        
        #pred : tensor([[39.12500, 27.54688, 69.50000, 73.81250,  0.67383,  0.00000]], device='cuda:0')  
        pred = torch.from_numpy(np.array(pred_data)).float().to(device)
        si = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--predictions', type=str, default=None, help='*.data path')
