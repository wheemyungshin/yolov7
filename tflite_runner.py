...
if __name__ == '__main__':
    #fd_model = tf.lite.Interpreter("../onnx2tf/saved_model/modified_seatbelt_all_AIchal_Otrain_mobilenet_s256_lrtune_e220_no_opt_128_128_integer_quant.tflite")
    #fd_model = tf.lite.Interpreter("../onnx2tf/saved_model/modified_seatbelt_face_mobilenet_s256_e090_no_opt_128_128_integer_quant.tflite")
    #fd_model = tf.lite.Interpreter("../onnx2tf/saved_model/modified_seatbelt_face_mobilenet_s256_e107_no_opt_128_128_integer_quant.tflite")
    #fd_model = tf.lite.Interpreter("../onnx2tf/saved_model/modified_seatbelt_face_mobilenet_s256_e170_no_opt_128_128_integer_quant.tflite")
    ...
    fd_model = tf.lite.Interpreter("../onnx2tf/saved_model/modified_seatbelt_face_balance4_mobilenet_s256_e180_no_opt_128_128_integer_quant.tflite")
    ...
    fd_model2= tf.lite.Interpreter("../onnx2tf/saved_model/NMS_mobilenet_2cls_s128_128_float32.tflite")
    #fd_model2= tf.lite.Interpreter("../onnx2tf/saved_model/NMS_mobilenet_s128_128_float32.tflite")
    ...
    #cap = cv2.VideoCapture("../data/pitta_QA_0531_sampling_crop/seatbelt2.mp4")

    vid_name = 'modified_seatbelt_face_balance4_little_stronger_mobilenet_s256_e180_day1_ww_s128_c04.mp4'
...
if __name__ == '__main__':
    fd_model = tf.lite.Interpreter("../onnx2tf/saved_model/modified_DK_ver03_e070_no_opt_192_192_integer_quant.tflite")
    
    
    fd_model2= tf.lite.Interpreter("../onnx2tf/saved_model/NMS_mobilenet_s192_192_float32.tflite")
    nms_part = tf.lite.Interpreter("weights_n78_tflite_nms_sep/nms_float32.tflite")
    fd_model.allocate_tensors()
    fd_model2.allocate_tensors()
    nms_part.allocate_tensors()
    ...
    vid_name = 'modified_DK_ver03_e070_no_opt_192_192_integer_quant.mp4'
        ...
        if frame is not None:
            ...
            for idx in range(len(scores)) :
                if scores[idx] > 0.1 :
                    size = (boxes[idx][2] - boxes[idx][0]) * (boxes[idx][3] - boxes[idx][1])
                    max_fd = boxes[idx]
                    max_fd[0] = int(max_fd[0] * (480 / 192))
                    max_fd[2] = int(max_fd[2] * (480 / 192))
                    max_fd[1] = int(max_fd[1] * (480 / 192))
                    max_fd[3] = int(max_fd[3] * (480 / 192))
                    max_fd = max_fd.astype(np.int32)
                    frame_vis = cv2.rectangle(frame_vis, (max_fd[0],max_fd[1]), (max_fd[2],max_fd[3]), (255,255,0), 2)            
                    cv2.putText(frame_vis, str(round(scores[idx], 5)), (max_fd[0],max_fd[1] - 2), 0, 1, [225, 255, 255], thickness=2, lineType=cv2.LINE_AA)
