...
def detect(save_img=False):
    ...
    print(imgsz)
    if opt.slice_inference:
        dataset = LoadImages(source, img_size=(imgsz[0]*3, imgsz[1]*3), stride=stride)
    else:
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if opt.slice_inference:
            ...
            print(seg_img.shape)
            seg_img = cv2.resize(seg_img, dsize=(imgsz[1], imgsz[0]))
            seg_img = np.transpose(seg_img, (2,0,1))
            seg_img = torch.from_numpy(seg_img).to(device)
            seg_img = seg_img.float()  # uint8 to fp16/32
            seg_img /= 255.0  # 0 - 255 to 0.0 - 1.0
        ...
        pred_seg, out = model_seg(seg_img, augment=opt.augment)
        proto = out[1]        
        ...
        if opt.slice_inference:
            ...
            print(img.shape)
            print(seg_img.shape)
            pred = model(seg_img, augment=opt.augment)[0]
            pred[0, :, 0] = pred[0, :, 0]*(img.shape[3]/seg_img.shape[3])
            pred[0, :, 1] = pred[0, :, 1]*(img.shape[2]/seg_img.shape[2])
            pred[0, :, 2] = pred[0, :, 2]*(img.shape[3]/seg_img.shape[3])
            pred[0, :, 3] = pred[0, :, 3]*(img.shape[2]/seg_img.shape[2])
            pred_concat = pred
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            for i, det in enumerate(pred_seg):  # detections per image
                if len(det):
                    masks = process_semantic_mask(proto[i], det[:, 6:], det[:, :6], seg_img.shape[2:], upsample=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--calc-optical-flow', action='store_true', help='calculate speed by optical flow')
    parser.add_argument('--nodebug', action='store_true', help='display only final voting results')
    parser.add_argument('--slice-inference', action='store_true', help='slice inference')
