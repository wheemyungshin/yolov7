...
CAMERA_ANGLE_WEIGHT = math.tan((CAMERA_LENS_ANGLE/180/2)*math.pi)*2 # 57 -> 1.0859113992768736
OVERALL_HEIGHT_DICT = { # meter
    0 : 1.8, # 차량류는 각도 등에 의해 박스 크기는 실제 전고보다 길 것을 반영하여 조금 더 크게 잡는다
    1 : 3.0, # 가장 차이 심함
    2 : 1.2, # 이제 사람은 포함하지 않으니 더 줄어듦
    4 : 1.6, # 길지 않아서 왜곡 정도 낮음, 그러나 자세 등으로 오히려 작아질 가능성 큼
}

def calculate_distance(cxywh, resolution):
    print(resolution)
    height = resolution[0]
    width = resolution[1]
    ...
    pixel_ratio = h / height

    print(CAMERA_ANGLE_WEIGHT)
    distance = (OVERALL_HEIGHT_DICT[c]) / (pixel_ratio) / CAMERA_ANGLE_WEIGHT # 거리 (mm)
    distance = distance / 1000 # 거리 (m)
...
def detect(save_img=False):
    ...
    colors = [[50,50,255], [50,255,50], [255,50,50], [50,255,255], [255,255,50], [255,50,255]]
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det[:, :6]):
                        ...
                        if save_img or view_img:  # Add bbox to image
                            distance_str = str(calculate_distance([int(cls), float(xywh[0]), float(xywh[1]), float(xywh[2]), float(xywh[3])], resolution=im0.shape)) + 'm'
                        ...
                        if cls == 4: # person
                            if xyxy[3] - xyxy[1] > max_person_size:
                                ...
                                closest_pedestrian_distance = calculate_distance([int(cls), float(xywh[0]), float(xywh[1]), float(xywh[2]), float(xywh[3])], resolution=im0.shape)
