    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            for lane_med_idx, lane_med in enumerate(lane_x_medians):
                lane_x_medians[lane_med_idx] = lane_x_medians[lane_med_idx]
            ...
            if lane_med_num_prev > 0 and lane_med_num_temp > 0:
                ...
                if lane_med_sum_prev < int((im0.shape[1]/5)*2/5) and int((im0.shape[1]/5)*3/5) < lane_med_sum_temp:
                    warn_status['crossline'] = [True, 30]
                elif lane_med_sum_temp < int((im0.shape[1]/5)*2/5) and int((im0.shape[1]/5)*3/5) < lane_med_sum_prev:
                    warn_status['crossline'] = [True, 30]
                else:
