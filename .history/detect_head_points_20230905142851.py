            ...
            if dataset.mode != 'image' and frame % frame_ratio != 0:
                ...
                if len(det):
                    ...
                    for xyxy_person in det[det[:, -1] == 0, :4]:
                        ...
                        if xyxy_person_np[2]- xyxy_person_np[0] < xyxy_person_np[3]- xyxy_person_np[1]:
                            is_long_body = True
                        else:
                            is_long_body = False
                        for xyxy_head in det[det[:, -1] == 1, :4]:
                            xyxy_head_np = xyxy_head.detach().cpu().numpy()
                            is_in_head = check_head_in_body(xyxy_head_np, xyxy_person_np, 15)
                            if is_in_head:
                                is_in_head_all = True
                        if not is_in_head_all and is_long_body:
