...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes, masks_unused) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            seg_path = str(path.resolve()).replace('images', 'segments').replace('.jpg', '.txt')
            print(seg_path)

            total_mask = torch.zeros((height, width))
            ...
            pred_masks_per_cls = torch.zeros((nc, masks.shape[0], masks.shape[1]), dtype=torch.long, device=device)
            semantic_gt_mask = torch.zeros((nc, masks.shape[0], masks.shape[1]), dtype=torch.long, device=device)
