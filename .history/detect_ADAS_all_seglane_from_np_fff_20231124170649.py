...
def detect(save_img=False):    
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            largest_area = 0
            smallest_area_limit = 0.01
            largest_mask = None
            for lane_mask in lane_masks:
                if largest_area/(image_masks.shape[0]*image_masks.shape[1]) > 0.03
            ...
            emptyth = 0.3
            if np.sum(prev_lane_going_lefts==1) > 5:
            ...
            if os.path.exists(os.path.join(det_source, p.name.split('/')[-1].split('_clean')[0]+'.txt')):
                ...
                if warn_status['going_left'][0] == True and warn_status['going_left'][1] == None :
