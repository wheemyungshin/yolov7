...
def predict_lane(net, input_img, size):
    ...
    all_cls_pred_dilated = np.zeros((1, 1, size[0], size[1]))
    for cls_idx in range(5):
        ...
        print(sig_output_mask.shape)
        print((size[1], size[0]))

        rows, cols = np.nonzero(sig_output_mask)
        for r, c in zip(rows, cols):
            sp = (int(c-vaf[r, c, 0]), int(r-vaf[r, c, 1]))
            ep = (int(c+vaf[r, c, 0]), int(r+vaf[r, c, 1]))
            cv2.line(sig_output_mask, sp, ep, 1, 1)

        sig_output_mask = cv2.resize(sig_output_mask, (size[1], size[0]), interpolation=cv2.INTER_CUBIC)
        sig_output_mask = np.expand_dims(sig_output_mask, axis=(0,1))
        '''

        print(np_sig_output.shape)
        print((size[1], size[0]))
        ...
        rows, cols = np.nonzero(sig_output_mask)
        for r, c in zip(rows, cols):
            sp = (int(c-vaf[r, c, 0]*10), int(r-vaf[r, c, 1]*10))
            ep = (int(c+vaf[r, c, 0]*10), int(r+vaf[r, c, 1]*10))
            cv2.line(sig_output_mask, sp, ep, 1, 1)
...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            image_masks = pred_masks_per_cls[-2].detach().cpu().numpy().astype(float)#[label_indexing]
            ...
            image_masks = all_cls_pred_dilated[0][0].detach().cpu().numpy().astype(float)#[label_indexing]
            image_masks = cv2.resize(image_masks, (vis_img.shape[1], vis_img.shape[0]), interpolation = cv2.INTER_NEAREST)
