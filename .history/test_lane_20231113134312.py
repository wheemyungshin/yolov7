...
def val(net, args, epoch):
    ...
    for b_idx, sample in enumerate(val_loader):
        ...
        if torch.sum(all_cls_target) > 0:
            ...
            miou[max_ious_idx].append(ious[max_ious_idx].detach().cpu().numpy())

        val_acc /= args.cls_num
        val_f1 /= args.cls_num

        epoch_acc.append(val_acc)
        epoch_f1.append(val_f1)

        cv2.imwrite('experiments/test/pred/'+str(b_idx)+'.jpg', (all_cls_pred_dilated.detach().cpu().numpy().reshape((72, 208))*255).astype(np.uint8))
        cv2.imwrite('experiments/test/mask/'+str(b_idx)+'.jpg', (all_cls_target.detach().cpu().numpy().reshape((72, 208))*255).astype(np.uint8))
        print('Done with image {} out of {}...'.format(min((b_idx+1), len(val_loader)), len(val_loader)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--bdd100k', action='store_true', default=False, help='the dataset is bdd100k')
    ...
    data_transforms = transforms.Compose([
                    ...
                    tf.GroupNormalize(mean=(data_mean, (0, )), std=(data_std, (1, ))),
                ])
    ...
    avg_acc, avg_f1 = val(model, args, 0)
