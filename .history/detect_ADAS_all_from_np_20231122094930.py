...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            lane_np = np.load(os.path.join(save_dir, 'mask_npy', 'lane', p.name.split('.')[0]+'_'+str(frame)))
            ...
            for i, det in enumerate(pred_seg):  # detections per image
                if len(det):
                    ...
                    save_road_np = np.save(os.path.join(save_dir, 'mask_npy', 'drivable_area', p.name.split('.')[0]+'_'+str(frame)))
