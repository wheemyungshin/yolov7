    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            if not compute_loss and opt_seg: #only bs 1 is possible
                ...
                vis_img
                conf = str(ious[max_ious_idx])
                tf = max(tl - 1, 1)  # font thickness
                t_size = cv2.getTextSize(conf, 0, fontScale=tl / 3, thickness=tf)[0]
                c1 = (int((face_result['bbox'][1]-face_margin )*vis_resize_ratio[0]), int((face_result['bbox'][2]-face_margin )*vis_resize_ratio[1]))
                c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
                cv2.rectangle(vis_frame, c1, c2, (0,0,0), -1, cv2.LINE_AA)  # filled
                cv2.putText(vis_frame, conf, (c1[0], c1[1] - 2), 0, tl / 3, [255, 200, 255], thickness=tf, lineType=cv2.LINE_AA)
