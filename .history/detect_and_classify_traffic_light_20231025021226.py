            ...
            if dataset.mode != 'image' and frame % frame_ratio != 0:
                ...
                if len(det):
                    ...
                    if opt.seg:
                        ...
                        np.save(os.path.join(save_dir, 'mask_npy', p.name.split('.')[0]+'_'+str(frame)), image_masks)
                    ...
                    for *xyxy, conf, cls in reversed(det[:, :6]):
                        if cls == 2: # traffic light
                            im_traffic_light = clean_im0[int(xyxy[1]):int(xyxy[3]), int(xyxy[0]):int(xyxy[2])].copy()
                            colorsum = np.sum(im_traffic_light, (0,1))
                            colormax_idx = np.argmax(colorsum)
                            print(colorsum.shape)
                            color_name = color_names[colormax_idx]
                            cv2.imwrite(os.path.join('im_traffic_light_samples', color_name+'_'+str(conf.item())+'.jpg'), im_traffic_light)
                        else:
                            color_name = ''
                        ...
                        if save_img or view_img:  # Add bbox to image
                            ...
                            label = f'{names[int(cls)]} {conf:.2f} {color_name}'
