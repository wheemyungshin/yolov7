                    ...
                    for xyxy_line_idx, xyxy_line in enumerate(xyxy_lines):
                        if len(xyxy_line.strip().split(' ')) == 5:
                            ...
                            if float(w) > max_traffic_light_size:
                                ...
                                #tracking
                                prev_result_matching_cls = np.array(prev_result)[np.array(prev_result)[:, 0]==cls]
                                iou_matrix = _iou(np.array([[int(x1_vis), int(y1_vis), int(x2_vis), int(y2_vis)]]), np.array(prev_result))
                                if 
