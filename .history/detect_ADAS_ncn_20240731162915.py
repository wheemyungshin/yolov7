                ...
                if len(det):
                    ...
                    for temp_box_id, (*xyxy, conf, cls) in enumerate(det[:, :6]):
                        ...
                        if temp_box_id in valid_idx:
                            ...
                            xyxy = np.array([
                                prev_weighted_value([prev_boxes_list[-prev_frame_idx][0] for prev_frame_idx if len(prev_boxes_list[-prev_frame_idx]) in range(max_prev_frame,0,-1)], xyxy[0].detach().cpu().float()),
                                prev_weighted_value([prev_boxes_list[-prev_frame_idx][1] for prev_frame_idx if len(prev_boxes_list[-prev_frame_idx]) in range(max_prev_frame,0,-1)], xyxy[1].detach().cpu().float()),
                                prev_weighted_value([prev_boxes_list[-prev_frame_idx][2] for prev_frame_idx if len(prev_boxes_list[-prev_frame_idx]) in range(max_prev_frame,0,-1)], xyxy[2].detach().cpu().float()),
                                prev_weighted_value([prev_boxes_list[-prev_frame_idx][3] for prev_frame_idx if len(prev_boxes_list[-prev_frame_idx]) in range(max_prev_frame,0,-1)], xyxy[3].detach().cpu().float()),
                                ])
                        else:
