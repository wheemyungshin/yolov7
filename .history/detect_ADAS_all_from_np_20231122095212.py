...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            lane_np = np.load(os.path.join(lane_source, p.name.split('.')[0]+'_'+str(frame)))
            ...
            road_np = np.load(os.path.join(road_source, p.name.split('.')[0]+'_'+str(frame)))
            
            vis_mask = im0.copy()
            for road_idx, image_mask_idx in enumerate(road_indices):
                vis_mask[road_np==image_mask_idx] = np.array(seg_colors[road_idx])
            alpha = 0.5
            im0 = cv2.addWeighted(im0, alpha, vis_mask, 1 - alpha, 0)
