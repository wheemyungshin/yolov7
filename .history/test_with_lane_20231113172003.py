...
def predict_lane(net, input_img, epoch):
    # do the forward pass
    outputs = net(input_img)[-1]
    
    all_cls_pred = torch.zeros_like(input_img.float())
...
def test(data,
    ...
    lane_model = ENet(heads=heads)

    if lane_weights is not None:
        pretrained_dict = {k: v for k, v in torch.load(lane_weights).items() if k in model.state_dict()
            and k not in ['hm.2.weight', 'hm.2.bias', 'haf.2.weight', 'haf.2.bias']}
        lane_model.load_state_dict(pretrained_dict, strict=False)
    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            lane_img = torch.from_numpy(lane_img).permute(2, 0, 1).contiguous().float().unsqueeze(0)
        
            pred_masks_per_cls = torch.zeros((nc+1, masks.shape[1], masks.shape[2]), dtype=torch.long, device=device)
            semantic_gt_mask = torch.zeros((nc+1, masks.shape[1], masks.shape[2]), dtype=torch.long, device=device)
            ...
            all_cls_pred_dilated = predict_lane(lane_model, lane_img, 0)

            pred_mask = torch.flatten(pred_masks_per_cls.float(), start_dim=1)
            pred_mask[-1] = all_cls_pred_dilated
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lane-weights', nargs='+', type=str, default='yolov7.pt', help='model.pt path(s)')
