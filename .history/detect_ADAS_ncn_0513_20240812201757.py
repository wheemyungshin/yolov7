...
def is_in_polygon(full_shape, roi_polygon, point):
    ...
    roi_mask = cv2.fillPoly(roi_mask, pts=[roi_polygon.astype(np.int32)], color=1)
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:        
        ...
        roi_polygon = [
            int(img_shape_bef_crop[1]*0.2), 0,
            int(img_shape_bef_crop[1]*0.8), 0,
            int(img_shape_bef_crop[1]*0.5), int(img_shape_bef_crop[0]*0.3)
        ]     
