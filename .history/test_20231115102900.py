...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            #print(pred[:, 4].cpu()) #class id
            #print(pred[:, 4].cpu().shape) # [300]
            #print(pred[:, 5].cpu()) #class id
            #print(tcls) #class id
            #print(pred[:, 5].cpu().shape) # [300]
            #print(len(tcls)) #n
            #print(torch.sum(correct))
            path_list.append(str(path.resolve()))
            #print(correct)
            #print((correct[pred[:, 4] > 0.3]))
            #print(torch.sum(correct))
            #print(torch.sum(correct[pred[:, 4] > 0.3]))
            print(torch.sum(correct[pred[:, 4] > 0.3]) / torch.sum(pred[:, 4] > 0.3))
            if torch.sum(correct[pred[:, 4] > 0.3]) / torch.sum(pred[:, 4] > 0.3) > 0.5:
    ...
    if len(stats) and stats[0].any():
        p, r, ap, f1, ap_class = ap_per_class(*stats, plot=plots, v5_metric=v5_metric, save_dir=save_dir, names=names, path_list=path_list)
        ap50, ap = ap[:, 0], ap.mean(1)  # AP@0.5, AP@0.5:0.95
        mp, mr, map50, map = p.mean(), r.mean(), ap50.mean(), ap.mean()
