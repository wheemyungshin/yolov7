...
if __name__ == '__main__':
    ...
    vid_writer = cv2.VideoWriter('n78_BC.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 8, (480, 480))

    for fileid, filename in enumerate(os.listdir(source_dir)):
    ...
    while True :
        ...
        for idx in range(len(scores)) :
            if scores[idx] > 0.3 :
                ...
                max_fd = boxes[idx]
                max_fd[0] = int(max_fd[0] * ( frame.shape[1] / 128))
                max_fd[2] = int(max_fd[2] * ( frame.shape[1] / 128))
                max_fd[1] = int(max_fd[1] * ( frame.shape[0] / 128))
                max_fd[3] = int(max_fd[3] * ( frame.shape[0] / 128))
