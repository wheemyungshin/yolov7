    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if quadrant == -1:
            ...
            for y_i in range(patch_num[0]):
                for x_i in range(patch_num[1]):
                    ...
                    print(pred.shape)
                    print(pred[:, is_body].shape)
                    print(pred[:, is_body, 0].shape)
                    print((pred[0, is_body, 0]-pred[0, is_body, 2]/2 > margin/4).shape)
                    print(pred[0, is_body][0, pred[0, is_body, 0]-pred[0, is_body, 2]/2 > margin/4].shape)

                    print(pred[:, is_face].shape)
                    print(pred[:, is_face, 0].shape)
                    print((pred[0, is_face, 0]-pred[0, is_face, 2]/2 > margin/4).shape)
                    print(pred[0, is_face][0, pred[0, is_face, 0]-pred[0, is_face, 2]/2 > margin/4].shape)
                    if x_i != 0:
                        pred_face = pred[0, is_face][0, pred[0, is_face, 0]-pred[0, is_face, 2]/2 > margin/4]
                        pred_body = pred[0, is_body][0, pred[0, is_body, 0]-pred[0, is_body, 2]/2 > margin]                        
                    if x_i != patch_num[1]-1:
                        pred_face = pred[0, is_face][0, pred[0, is_face, 0]+pred[0, is_face, 2]/2 < patch.shape[3]-margin/4]
                        pred_body = pred[0, is_body][0, pred[0, is_body, 0]+pred[0, is_body, 2]/2 < patch.shape[3]-margin]
                    if y_i != 0:
                        pred_face = pred[0, is_face][0, pred[0, is_face, 1]-pred[0, is_face, 3]/2 > margin/4]
                        pred_body = pred[0, is_body][0, pred[0, is_body, 1]-pred[0, is_body, 3]/2 > margin]
                    if y_i != patch_num[0]-1:
                        pred_face = pred[0, is_face][0, pred[0, is_face, 1]-pred[0, is_face, 3]/2 < patch.shape[2]-margin/4]
                        pred_body = pred[0, is_body][0, pred[0, is_body, 1]-pred[0, is_body, 3]/2 < patch.shape[2]-margin]
            ...
            if x_i != 0:
                pred = pred[:, pred[0, :, 0]-pred[0, :, 2]/2 > margin/4]
            if x_i != patch_num[1]-1:
                pred = pred[:, pred[0, :, 0]+pred[0, :, 2]/2 < patch.shape[3]-margin/4]
            if y_i != 0:
            ...
            pred_prev[quadrant] = pred
            for quadrant_idx in range((patch_num[0]*patch_num[1])):
                if quadrant_idx != quadrant and pred_prev[quadrant_idx] is not None:
                    pred = torch.cat((pred, pred_prev[quadrant_idx]), 1)
        ...
        for y_i in range(patch_num[0]):
            for x_i in range(patch_num[1]):
                ...
                if y_i != patch_num[0]-1:
                    pred = pred[:, pred[0, :, 1]+pred[0, :, 3]/2 < patch.shape[2]-margin/4]
                pred_raw[0][pred_raw[0][:,-1]==1]
        '''

        pred_list.append(pred_raw[0])
