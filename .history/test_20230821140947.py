    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            if opt.seg: #only bs 1 is possible
                ...
                pred_mask = torch.flatten(torch.unsqueeze(masks.float() , 0), start_dim=1)
                pred_mask = seg_pred.gt_(0.5).float()
                ious = mask_iou(pred_mask, torch.flatten(masks, start_dim=1))
                print(ious)
                print(ious.shape)
