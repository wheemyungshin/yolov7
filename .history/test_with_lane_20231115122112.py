...
from utils.general import check_dataset, check_file, check_img_size, check_requirements, \
    box_iou, non_max_suppression, scale_coords, set_logging, increment_path, colorstr, non_max_suppression_seg, \
...
def test(data,
    ...
    dataloader = create_dataloader(data[task], imgsz, batch_size, gs, opt, pad=0.0, rect=True,
                                    prefix=colorstr(f'{task}: '), valid_idx=None, load_seg=True)[0]
    ...
    print("drivable area + lane : ", sum(miou)/len(miou))
