...
def investigate_prev_list(input_list, item, maxiter):
    for i_ in range(maxiter):
        i = -i-1
        item = input_list[i][item]
    return item
...
def detect(save_img=False):    
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        prev_distances = prev_distances_list[-1]
                    ...
                    for temp_box_id, (*xyxy, conf, cls) in enumerate(det[:, :6]):
                        ...
                        if temp_box_id in valid_idx:
                            ...
                            for prev_frame_idx in range(max_prev_frame,0,-1):
                                prev_box_id = investigate_prev_list(match_box_ids_y_list, temp_box_id, maxiter=prev_frame_idx)
                                prev_distance = investigate_prev_list(prev_distances_list, prev_box_id, maxiter=prev_frame_idx)
                                distance_weights.append(prev_distance*prev_frame_idx/max_prev_frame)
                            distance = round(sum(distance_weights)*0.34 + temp_distance*0.66, 3)
