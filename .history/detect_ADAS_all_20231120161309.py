...
def detect(save_img=False):
    warn_status = {
        'last_light' : [['Nolight', 'Nolight', 'Nolight', 'Nolight', 'Nolight'], 0],
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            print(lane_x_medians)
            print(warn_status['crossline'])
            ...
            for i, det in enumerate(pred):  # detections per image
                    ...
                    for *xyxy, conf, cls in reversed(det[:, :6]):
                        if cls in [2, 4, 6, 7, 8, 10, 11]:
                            ...
                            if last_light != 'Nolight'
                                for lane_med_idx in range(len(warn_status['last_light'][0])-1):
                                    warn_status['last_light'][0][lane_med_idx] = warn_status['last_light'][0][lane_med_idx+1]
                                warn_status['last_light'][0][-1] = last_light
                                warn_status['last_light'][1] = 120
                ...
                if warn_status['last_light'][1] <= 0 : 
                    warn_status['last_light'][0] = [['Nolight', 'Nolight', 'Nolight', 'Nolight', 'Nolight'], 0]
                else:
                ...
                for warn_key in warn_status.keys():
                    if warn_key == 'last_light':
                        count_red = warn_status[warn_key][0].count('Red')
                        count_arrow = warn_status[warn_key][0].count('Arrow')
                        count_green = warn_status[warn_key][0].count('Green')
                        if warn_status[warn_key][0].count('Nolight') == len(warn_status['last_light'][0]):
                            ...
                            if count_red > count_arrow and count_red > count_green:
                                print_color = [0, 0, 255]
                            elif count_red > count_arrow and count_red > count_green:
