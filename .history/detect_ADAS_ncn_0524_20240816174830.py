...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        pred_lane[0][:, 2] = pred_lane[0][:, 2] + crop_x1
            ...
            int(im0s.shape[1]*0.1), int(im0s.shape[0]),
            int(im0s.shape[1]*0.9), int(im0s.shape[0]),
            int(im0s.shape[1]*0.5), int(im0s.shape[0]*0.6)
        right_xyxy = [im0s.shape[1]*0.1, im0s.shape[0]*0.55, im0s.shape[1]*0.4, im0s.shape[0]]
