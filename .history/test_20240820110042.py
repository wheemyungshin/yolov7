    ...
    if training:  # called by train.py
        ...
        if opt.cfg:
            ...
            if type(ckpt_weights['model']) is Model:
                state_dict = ckpt_weights['model'].float().state_dict().detach()  # to FP32
            elif type(ckpt_weights['model']) is collections.OrderedDict:
                state_dict = ckpt_weights['model'].detach()  # to FP32
            else:
            ...
            ckpt = Model(opt.cfg, ch=3, nc=nc, qat=opt.qat).to(device)  # create#, nm=nm).to(device)  # create
            #if opt.qat:
            #    fuse_modules(ckpt)
