...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            if os.path.exists(os.path.join(lane_source, p.name.split('/')[-1].split('_clean')[0]+'.npy')):
                lane_np = np.load(os.path.join(lane_source, p.name.split('/')[-1].split('_clean')[0]+'.npy'))
            ...
            if os.path.exists(os.path.join(road_source, p.name.split('/')[-1].split('_clean')[0]+'.npy')):
                road_np = np.load(os.path.join(road_source, p.name.split('/')[-1].split('_clean')[0]+'.npy'))
            ...
            if os.path.exists(os.path.join(det_source, p.name.split('/')[-1].split('_clean')[0]+'.txt')):
                with open(os.path.join(det_source, p.name.split('/')[-1].split('_clean')[0]+'.txt'), 'r') as f:
                    ...
                    for xyxy_line_idx, xyxy_line in enumerate(xyxy_lines):
                        if len(xyxy_line.strip().split(' ')) == 5:
                            ...
                            if float(w) > max_traffic_light_size:
                                ...
                                if cls == 4: # person
                                    if float(h) > max_person_size:
                                        max_person_size = float(h)
                                        closest_pedestrian_distance = calculate_distance([int(cls), 
                                            float(x)*im0.shape[1], float(y)*im0.shape[0], float(w)*im0.shape[1], float(h)*im0.shape[0]])
                    ...
                    if save_img:
                        if vid_path != save_path.split('_clean')[0].split('_'+save_path.split('_clean')[0].split('_')[-1])[0][:-1]:  # new video
                            vid_path = save_path.split('_clean')[0].split(save_path.split('_clean')[0].split('_')[-1])[0][:-1]
