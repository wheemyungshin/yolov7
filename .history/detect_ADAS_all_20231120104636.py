...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            lane_x_median = int(im0.shape[0]/2)
            for cls_idx in range(5):
                ...
                lane_roi = cls_pred_dilated[int(image_masks.shape[0]/2):, int(image_masks.shape[1]/3):int(image_masks.shape[1]*2/3)]

                if cls_idx == 3: # yellow line
                    lane_xs, lane_ys = np.nonzero(cls_pred_dilated)
                    if len(lane_xs) > 0:
