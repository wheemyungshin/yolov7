...
def detect(save_img=False):
    ...
    data_idx = -1
    real_run_idx = -1
    pred_prev = {0: None, 1: None, 2: None, 3: None}
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if data_idx % frame_ratio != 0:
            pred = pred_result.copy()
        else:
            real_run_idx+=1
            if opt.all_quadrant_at_once:
                quadrant = -1
            else:
                quadrant = real_run_idx % 4      
            ...
            patch_num = 2
            margin = 128
            patch_stride_x = int((img.shape[3]-margin) / patch_num)
            patch_stride_y = int((img.shape[2]-margin) / patch_num)
            if quadrant == 0:
                x_i, y_i = 1, 0
            elif quadrant == 1:
                x_i, y_i = 0, 0
                ...
                for x_i in range(patch_num):
                    for y_i in range(patch_num):
                        x1 = x_i * patch_stride_x
                        y1 = y_i * patch_stride_y
                        x2 = x1+patch_stride_x+margin
                        y2 = y1+patch_stride_y+margin
                        patch = img[:, :, y1:y2, x1:x2]
                        pred_ = model(patch, augment=opt.augment)[0]
                        pred_[0, :, 0] = pred_[0, :, 0] + x1
                        pred_[0, :, 1] = pred_[0, :, 1] + y1
                        if pred_concat is None:
                            pred_concat = pred_
                        else:
                            pred_concat = torch.cat((pred_concat, pred_), 1)
            ...
            if quadrant != -1:
                ...
                if quadrant == 0:
                    pred_ = pred_[:, pred_[0, :, 0]-pred_[0, :, 2]/2 > margin/4]
                    pred_ = pred_[:, pred_[0, :, 1]+pred_[0, :, 3]/2 < patch.shape[2]-margin/4]
                elif quadrant == 1:
                    pred_ = pred_[:, pred_[0, :, 0]+pred_[0, :, 2]/2 < patch.shape[3]-margin/4]
                    pred_ = pred_[:, pred_[0, :, 1]+pred_[0, :, 3]/2 < patch.shape[2]-margin/4]
                elif quadrant == 2:
                    pred_ = pred_[:, pred_[0, :, 0]+pred_[0, :, 2]/2 < patch.shape[3]-margin/4]
                    pred_ = pred_[:, pred_[0, :, 1]-pred_[0, :, 3]/2 > margin/4]
                elif quadrant == 3:
                    pred_ = pred_[:, pred_[0, :, 0]-pred_[0, :, 2]/2 > margin/4]
                    pred_ = pred_[:, pred_[0, :, 1]-pred_[0, :, 3]/2 > margin/4]
                pred_[0, :, 0] = pred_[0, :, 0] + x1
                ...
                pred_prev[quadrant] = pred_
                pred_concat = pred_
                for quadrant_idx in range(4):
                    if quadrant_idx != quadrant and pred_prev[quadrant_idx] is not None:
                        pred_concat = torch.cat((pred_concat, pred_prev[quadrant_idx]), 1)

            pred_raw = non_max_suppression(pred_concat, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms)
            ...
            pred_result = [torch.cat(pred_list, 0)]
        
        t3 = time_synchronized()

        # Process detections
        for i, det in enumerate(pred.copy()):  # detections per image
            ...
            print(f'{s}Done. ({(1E3 * (t3 - t1)):.1f}ms) All')
