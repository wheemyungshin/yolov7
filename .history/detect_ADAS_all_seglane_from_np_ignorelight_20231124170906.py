...
def detect(save_img=False):    
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if (save_path.split('_clean')[0].split('_'+save_path.split('_clean')[0].split('_')[-1])[0]).split('/')[-1] != 'no_helmet_light':
            continue

        print((save_path.split('_clean')[0].split('_'+save_path.split('_clean')[0].split('_')[-1])[0]).split('/')[-1])
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            if os.path.exists(os.path.join('runs/detect/test_gt_nohelmet_split/labels', 'no_helmet_light_'+str(frame_by_name)+'.txt')):
                print(os.path.join('runs/detect/test_gt_nohelmet_split/labels', 'no_helmet_light_'+str(frame_by_name)+'.txt'))
                with open(os.path.join('runs/detect/test_gt_nohelmet_split/labels', 'no_helmet_light_'+str(frame_by_name)+'.txt'), 'r') as f:
                    xyxy_lines = f.readlines()
                    print(xyxy_lines)
                    for xyxy_line_idx, xyxy_line in enumerate(xyxy_lines):
                        if len(xyxy_line.strip().split(' ')) == 5:
                            ...
                            if cls == 1:
                                continue
                            if frame_by_name > 200 and float(x) < 0.5:
                                continue
                            if frame_by_name > 230 and float(w) > 0.055:
                                continue
                            if float(y) < 0.2:
                                continue
                            ...
                            distance = str(calculate_distance([4, float(x), float(y), float(w), float(h)]))
                            class_name = class_name_dict[4]
            ...
            if os.path.exists(os.path.join(det_source, p.name.split('/')[-1].split('_clean')[0]+'.txt')):
                ...
                with open(os.path.join(det_source, p.name.split('/')[-1].split('_clean')[0]+'.txt'), 'r') as f:
                    ...
                    for xyxy_line_idx, xyxy_line in enumerate(xyxy_lines):
                        if len(xyxy_line.strip().split(' ')) == 5:
                            ...
                            if frame_by_name < 260 and (save_path.split('_clean')[0].split('_'+save_path.split('_clean')[0].split('_')[-1])[0]).split('/')[-1] == 'no_helmet_crossline':
                                
                                if float(w) > float(h):
                                    print(float(w), float(h))
                                    continue

                                if cls == 7 and float(h) > 0.07:
                                    continue
                            ...
                            if 280 < frame_by_name < 330 and (save_path.split('_clean')[0].split('_'+save_path.split('_clean')[0].split('_')[-1])[0]).split('/')[-1] == 'no_helmet_crossline':
                                if cls not in [4, 6]:
                                    continue
                            ...
                            if cls !=2 and (float(h)*im0.shape[0])*(float(w)*im0.shape[1]) < 1600:
                                continue
                            if cls ==2 and (float(h)*im0.shape[0])*(float(w)*im0.shape[1]) < 700:
                                continue
                            #tracking
                            '''
                            prev_result_np = np.array(prev_result)
                ...
                if save_img:
                    ...
                    if opt.save_frame:
                        cv2.imwrite(os.path.join(save_dir, 'vis_frames', p.name.split('_clean')[0])+'.jpg', im0)
                        cv2.imwrite(os.path.join(save_dir, 'images', p.name.split('_clean')[0])+'_clean.jpg', clean_im0)
