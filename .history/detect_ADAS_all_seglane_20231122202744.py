...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            print(prev_lane_on_lefts)
            if np.sum(prev_lane_on_lefts[prev_lane_on_lefts!=-1]) > len(prev_lane_on_lefts)*0.7:
                warn_status['on_left'][0] = True
            elif np.sum(prev_lane_on_lefts[prev_lane_on_lefts!=-1]) < len(prev_lane_on_lefts)*0.3:
                warn_status['on_left'][0] = False
            else:
                warn_status['on_left'][0] = None
            if np.sum(temp_lane_on_lefts[temp_lane_on_lefts!=-1]) > len(temp_lane_on_lefts)*0.7:
                warn_status['on_left'][1] = True
            elif np.sum(temp_lane_on_lefts[temp_lane_on_lefts!=-1]) < len(temp_lane_on_lefts)*0.3:
            ...
            if np.sum(prev_lane_going_lefts[prev_lane_going_lefts!=-1]) > len(prev_lane_going_lefts)*0.7:
                warn_status['on_left'][0] = True
            elif np.sum(prev_lane_going_lefts[prev_lane_going_lefts!=-1]) < len(prev_lane_going_lefts)*0.3:
                warn_status['on_left'][0] = False
            else:
                warn_status['on_left'][0] = None
            if np.sum(temp_lane_going_lefts[temp_lane_going_lefts!=-1]) > len(temp_lane_going_lefts)*0.7:
                warn_status['on_left'][1] = True
            elif np.sum(temp_lane_going_lefts[temp_lane_going_lefts!=-1]) < len(temp_lane_going_lefts)*0.3:
                warn_status['on_left'][1] = False
            else:
                warn_status['on_left'][1] = None
