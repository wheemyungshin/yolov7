...
def is_in_polygon(full_shape, roi_polygon, point):
    ...
    result_points = []
    count = 0
    if roi_mask[int(point[1]), int(point[0])] == 1:
        return True
    else:
        return False
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:        
        ...
        roi_polygon = [
            tuple([int(img_shape_bef_crop[1]*0.2), 0]),
            tuple([int(img_shape_bef_crop[1]*0.8), 0]),
            tuple([int(img_shape_bef_crop[1]*0.5), int(img_shape_bef_crop[0]*0.3)])
        ]     
        ...
        cv2.line(im0, (int(roi_polygon[0][0]),
                        int(roi_polygon[0][1])), 
                        (int(roi_polygon[1][0]),
                        int(roi_polygon[1][1])),
        ...
        cv2.line(im0, (int(roi_polygon[0][0]),
                        ...
                        int(roi_polygon[2][1])),
                        (255,0,255), thickness=1) 
        cv2.line(im0, (int(roi_polygon[1][0]),
                        int(roi_polygon[1][1])),
                        (int(roi_polygon[2][0]),
                        int(roi_polygon[2][1])),
                        (255,0,255), thickness=1) 
                    ...
                    for temp_box_id, (*xyxy, conf, cls) in enumerate(det[:, :6]):
                        ...
                        if save_img or view_img:  # Add bbox to image
                            ...
                            if opt.debug:
                                ...
                                if is_in_polygon(im0.shape[:2], roi_polygon, [(xyxy[0]+xyxy[2])/2, (xyxy[1]+xyxy[3])/2]):
