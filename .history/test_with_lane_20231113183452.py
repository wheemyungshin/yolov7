...
def test(data,
    ...
    lane_models = ENet(heads=heads)

    if lane_weights is not None:
        pretrained_dict = {k: v for k, v in torch.load(lane_weights).items() if k in lane_model.state_dict()
    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            all_cls_pred_dilated = predict_lane(lane_model, lane_img, (masks.shape[1], masks.shape[2]))
