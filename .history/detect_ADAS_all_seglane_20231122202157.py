...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            prev_lane_on_lefts = lane_on_lefts[:int(len(lane_on_lefts)/2)]
            temp_lane_on_lefts = lane_on_lefts[int(len(lane_on_lefts)/2):]
            if sum(prev_lane_on_lefts) > len(prev_lane_on_lefts)*0.7:
                warn_status['on_left'][0] = True
            elif sum(prev_lane_on_lefts) < len(prev_lane_on_lefts)*0.3:
                warn_status['on_left'][0] = False
            else:
                warn_status['on_left'][0] = None
            if sum(temp_lane_on_lefts) > len(temp_lane_on_lefts)*0.7:
                warn_status['on_left'][1] = True
            elif sum(temp_lane_on_lefts) < len(temp_lane_on_lefts)*0.3:
                warn_status['on_left'][1] = False
            else:
                warn_status['on_left'][1] = None

            prev_lane_going_lefts = lane_going_lefts[:int(len(lane_going_lefts)/2)]
            temp_lane_going_lefts = lane_going_lefts[int(len(lane_going_lefts)/2):]
            if sum(prev_lane_going_lefts) > len(prev_lane_going_lefts)*0.7:
                warn_status['on_left'][0] = True
            elif sum(prev_lane_going_lefts) < len(prev_lane_going_lefts)*0.3:
                warn_status['on_left'][0] = False
            else:
                warn_status['on_left'][0] = None
            if sum(temp_lane_going_lefts) > len(temp_lane_going_lefts)*0.7:
                warn_status['on_left'][1] = True
            elif sum(temp_lane_going_lefts) < len(temp_lane_going_lefts)*0.3:
                warn_status['on_left'][1] = False
            else:
                warn_status['on_left'][1] = None
