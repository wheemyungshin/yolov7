...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if len(lines_leftxyxy_to_draw) == 0:
            lines_leftxyxy_to_draw = default_left_xyxy
        if len(lines_rightxyxy_to_draw) == 0:
            lines_rightxyxy_to_draw = default_right_xyxy
        ...
        # prev tracking
        left_xyxy = [l_item*0.3 + prev_l_item*0.7 for l_item, prev_l_item in zip(left_xyxy, lines_leftxyxy_to_draw)]
        right_xyxy = [r_item*0.3 + prev_r_item*0.7 for r_item, prev_r_item in zip(right_xyxy, lines_rightxyxy_to_draw)]
