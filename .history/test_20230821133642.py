...
from utils.general import coco80_to_coco91_class, check_dataset, check_file, check_img_size, check_requirements, \
    box_iou, non_max_suppression, scale_coords, xyxy2xywh, xywh2xyxy, set_logging, increment_path, colorstr, non_max_suppression_seg, mask_iou
    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            if opt.seg:
                ...
                seg_pred = process_semantic_mask(proto[si], det[:, 6:], det[:, :6], img.shape[2:], upsample=True)
