...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if quadrant == 0:
            print(pred[0].shape)
            print((pred[0, :, 0]+pred[0, :, 2]/2 > margin/4).shape)
            print(pred[0, pred[0, :, 0]+pred[0, :, 2]/2 > margin/4].shape)
            pred = pred[:, pred[0, :, 0]+pred[0, :, 2]/2 > margin/4]
            pred = pred[:, pred[0, :, 1]+pred[0, :, 3]/2 < patch.shape[0]-margin/4]
        elif quadrant == 1:
            pred[0] = pred[0, pred[0, :, 0]+pred[0, :, 2]/2 < patch.shape[1]-margin/4]
            pred[0] = pred[0, pred[0, :, 1]+pred[0, :, 3]/2 < patch.shape[0]-margin/4]
        elif quadrant == 2:
            pred[0] = pred[0, pred[0, :, 0]+pred[0, :, 2]/2 < patch.shape[1]-margin/4]
            pred[0] = pred[0, pred[0, :, 1]+pred[0, :, 3]/2 > margin/4]
        elif quadrant == 3:
            pred[0] = pred[0, pred[0, :, 0]+pred[0, :, 2]/2 > margin/4]
            pred[0] = pred[0, pred[0, :, 1]+pred[0, :, 3]/2 > margin/4]
        pred[0, :, 0] = pred[0, :, 0] + x1
        pred[0, :, 1] = pred[0, :, 1] + y1
        '''
        for quadrant_idx in range(4):
            if quadrant_idx != quadrant:
