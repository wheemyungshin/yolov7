        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if dataset.mode != 'image' and frame % frame_ratio != 0:
                ...
                if len(det):
                    ...
                    # Mask plotting ----------------------------------------------------------------------------------------
                    os.makedirs('det_npy', exist_ok=True)
                    np.save(os.path.join('det_npy', p.name.split('.')[0]), det.detach().cpu().numpy())

                    np_data = np.load(os.path.join('det_npy', p.name.split('.')[0]), det.detach().cpu().numpy())
