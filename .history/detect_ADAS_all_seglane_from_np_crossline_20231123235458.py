...
def detect(save_img=False):    
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            print(os.path.join('runs/detect/test_gt_nohelmet_crossline_split/labels', 'no_helmet_light_'+str(frame_by_name)+'.txt'))
            if os.path.exists(os.path.join('runs/detect/test_gt_nohelmet_crossline_split/labels', 'no_helmet_light_'+str(frame_by_name)+'.txt')):
                with open(os.path.join('runs/detect/test_gt_nohelmet_crossline_split/labels', 'no_helmet_light_'+str(frame_by_name)+'.txt'), 'r') as f:
                    xyxy_lines = f.readlines()
