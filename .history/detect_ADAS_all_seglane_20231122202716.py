...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            if np.sum(prev_lane_going_lefts) > len(prev_lane_going_lefts)*0.7:
                warn_status['on_left'][0] = True
            elif np.sum(prev_lane_going_lefts) < len(prev_lane_going_lefts)*0.3:
                warn_status['on_left'][0] = False
            else:
                warn_status['on_left'][0] = None
            if np.sum(temp_lane_going_lefts) > len(temp_lane_going_lefts)*0.7:
                warn_status['on_left'][1] = True
            elif np.sum(temp_lane_going_lefts) < len(temp_lane_going_lefts)*0.3:
                warn_status['on_left'][1] = False
            else:
                warn_status['on_left'][1] = None
