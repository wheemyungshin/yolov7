    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            if lane_med_num_prev > 0 and lane_med_num_temp > 0:
                ...
                if lane_med_sum_prev < int(image_masks.shape[1]*5/11) and int(image_masks.shape[1]*6/11) < lane_med_sum_temp:
                    warn_status['crossline'] = [True, 30]
                elif lane_med_sum_temp < int(image_masks.shape[1]*5/11) and int(image_masks.shape[1]*6/11) < lane_med_sum_prev:
                    warn_status['crossline'] = [True, 30]
                else:
                    warn_status['crossline'] = [False, 0]
            else:
                warn_status['crossline'] = [False, 0]
            ...
            if prev_gray is not None:
                ...
                warn_status['speed'] = np.mean(global_value)*50
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                for warn_key in warn_status.keys():
                    ...
                    if warn_key == 'speed':
                        print_value = warn_key + ' : ' + warn_status[warn_key]
                        print_color = [255, 255, 255]
