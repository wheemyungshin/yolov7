        ...
        if hyp is not None and hyp.get('render_phone', None) is not None:
            ...
            for folder in phone_folders :
                if len(folder.split(".")) == 1 :
                    files = os.listdir(os.path.join(hyp.get('render_phone', None), folder))
                    for file in files :
                        # 너무 수직인 각도면 거를 것
        ...
        if hyp is not None and random.random() < hyp.get('render_phone', ['', 0.0])[1]:
            if len(labels) > 0:
                ...
                for idx in range(num_of_phone) :
                    ...
                    phone_img = cv2.resize(phone_img, None, fx=0.2+random.random()*0.1, fy=0.2+random.random()*0.1, interpolation=cv2.INTER_LINEAR)
                    if random.random() > 0.5 :
                    ...
                    if img.shape[1]-phone_img.shape[1] > 0 and img.shape[0]-phone_img.shape[0] > 0:
                        ...
                        zero_img = np.zeros_like(phone_img)
                        zero_img[phone_img<220] = 1
                        non_zero_indices = np.nonzero(zero_img)
                        if len(non_zero_indices[0]) > 0 and len(non_zero_indices[1]) > 0:
                            ...
                            face_img_crop_pillow = Image.fromarray(face_img_crop)
                            blended_pillow = Image.alpha_composite(face_img_crop_pillow, phone_img_pillow)
                            blended_img=np.array(blended_pillow)  

                            blended_img = cv2.cvtColor(blended_img, cv2.COLOR_RGBA2RGB)
					        face_img[phone_img_position_y:phone_img_position_y+phone.shape[0], phone_img_position_x:phone_img_position_x+phone.shape[1]] = blended_img
                            ...
                            max_x = np.max(non_zero_indices[1])
                            max_y = np.max(non_zero_indices[0])
                            new_label = np.array([[3, phone_img_position_x+min_x, phone_img_position_y+min_y, phone_img_position_x+max_x, phone_img_position_y+max_y]])
                            new_segment = np.array([
                                [phone_img_position_x+min_x, phone_img_position_y+min_y], 
                                [phone_img_position_x+max_x, phone_img_position_y+min_y],  
                                [phone_img_position_x+max_x, phone_img_position_y+max_y],
                                [phone_img_position_x+min_x, phone_img_position_y+max_y]
                                ])
