        ...
        if hyp is not None and random.random() < hyp.get('render_phone', ['', 0.0])[1]:
            if len(labels) > 0:
                ...
                for idx in range(num_of_phone) :
                    ...
                    if random.random() > 0.5 :
						phone = cv2.rotate(phone, cv2.ROTATE_90_CLOCKWISE)
                    ...
                    if img.shape[1]-phone_img.shape[1] > 0 and img.shape[0]-phone_img.shape[0] > 0:
                        ...
                        if len(non_zero_indices[0]) > 0 and len(non_zero_indices[1]) > 0:
                            ...
                            img_crop[phone_img<220] = phone_img[phone_img<220]
                            img[phone_img_position_y:phone_img_position_y+phone_img.shape[0], phone_img_position_x:phone_img_position_x+phone_img.shape[1]] = img_crop
