...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if hyp is not None and random.random() < hyp.get('render_phone', ['', 0.0])[1]:
            num_of_hand_img = [1,4]
            num_of_hand = random.randint(num_of_hand_img[0], num_of_hand_img[1])
            hand_imgs = os.listdir(hyp.get('render_phone', None)[0])
            labels_after_filter = []
            segments_after_filter = []
            for idx in range(num_of_hand) :
                hand_img = cv2.imread(os.path.join(hyp.get('render_phone', None)[0], hand_imgs[random.randint(0, len(hand_imgs) - 1)]), cv2.IMREAD_UNCHANGED)
                hand_img = cv2.resize(hand_img, None, fx=0.2+random.random()*0.1, fy=0.2+random.random()*0.1, interpolation=cv2.INTER_LINEAR)
                ...
                if img.shape[1]-hand_img.shape[1] > 0 and img.shape[0]-hand_img.shape[0] > 0:
                    ...
                    new_label = np.array([3, hand_img_position_x, hand_img_position_y, hand_img_position_x+hand_img.shape[1], hand_img_position_y+hand_img.shape[0]])
                    new_label[1] = hand_img_position_x
                    new_label[2] = hand_img_position_y
                    new_label[3] = hand_img_position_x+hand_img.shape[1]
                    new_label[4] = hand_img_position_y+hand_img.shape[0]
                    labels_after_filter.append(new_label)
                    
                    new_segment = np.array([
                        [hand_img_position_x, hand_img_position_y], 
                        [hand_img_position_x+hand_img.shape[1], hand_img_position_y],  
                        [hand_img_position_x+hand_img.shape[1], hand_img_position_y+hand_img.shape[0]],
                        [hand_img_position_x, hand_img_position_y+hand_img.shape[0]]
                        ])
                    segments_after_filter.append(new_segment)
