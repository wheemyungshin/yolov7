...
def random_distortion(im, label, size):
    print(label)

    k1 = random.random()-0.5
    k2 = random.random() * 0.3
    k3 = 0
    ...
    xyxys = label[:, 1:].copy()
    xyxys[:, 0] = (label[:, 1] - label[:, 3]*0.5) * w
    xyxys[:, 1] = (label[:, 2] - label[:, 4]*0.5) * h
    xyxys[:, 2] = (label[:, 1] + label[:, 3]*0.5) * w 
    xyxys[:, 3] = (label[:, 2] + label[:, 4]*0.5) * h

    for xyxy in xyxys:
        x1 = max(min(int(xyxy[0]), shape[1]-1), 0)
        y1 = max(min(int(xyxy[1]), shape[0]-1), 0)
        x2 = max(min(int(xyxy[2]), shape[1]-1), 0)
        y2 = max(min(int(xyxy[3]), shape[0]-1), 0)
        x1_dist = (mapx[y1, x1]+mapx[y2, x1])/2
        y1_dist = (mapy[y1, x1]+mapy[y1, x2])/2
        x2_dist = (mapx[y1, x2]+mapx[y2, x2])/2
        y2_dist = (mapy[y2, x1]+mapy[y2, x2])/2
        x_dist = (x1_dist + x2_dist)/2
        y_dist = (y1_dist + y2_dist)/2
        w_dist = x2_dist - x1_dist
        h_dist = y2_dist - y1_dist
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            img, labels = random_distortion(img, labels, (h, w))
...
def load_image_and_label(self, index, ratio_maintain=True, hyp=None):
    ...
    if img is None:  # not cached
        ...
        if 'n78' not in path:
            ...
            img[:, :, 1] = img[:, :, 1] + random.randint(4, 5)
            if random.random() < 0.5:
