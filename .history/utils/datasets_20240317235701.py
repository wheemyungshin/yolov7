...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # Load image
            img, (h0, w0), (h, w), (h_orig, w_orig) = load_image(self, index, ratio_maintain=self.ratio_maintain)
            ...
            labels[:, 1] = labels[:, 1] * (w_orig / w0)
            labels[:, 2] = labels[:, 2] * (h_orig / h0)
            labels[:, 3] = labels[:, 3] * (w_orig / w0)
            labels[:, 4] = labels[:, 4] * (h_orig / h0)
...
def load_image(self, index, ratio_maintain=True):
    ...
    if img is None:  # not cached
        ...
        if ratio_maintain:
            if isinstance(self.img_size, tuple):
                ...
                if r != 1:  # always resize down, only resize up if training with augmentation
                    interp = cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR
                    img = cv2.resize(img, (int(w0 * r), int(h0 * r)), interpolation=interp)
            return img, (h0, w0), img.shape[:2], (height, width)  # img, hw_original, hw_resized
        else:
            if isinstance(self.img_size, tuple):
        ...
        img_concat[:height, :width] = img
        img_concat[height:, :noise_w] = noise_img
        return img_concat, self.img_hw0[index], self.img_hw[index], (height, width)  # img, hw_original, hw_resized
...
def load_mosaic9(self, hyp, index):
    ...
    for i, index in enumerate(indices):
        # Load image
        img, (h0_, w0_), (h, w), (h_orig, w_orig) = load_image(self, index, ratio_maintain=self.ratio_maintain)
        ...
        # Labels
        labels, segments = self.labels[index].copy(), self.segments[index].copy()
        labels[:, 1] = labels[:, 1] * (w_orig / w0_)
        labels[:, 2] = labels[:, 2] * (h_orig / h0_)
        labels[:, 3] = labels[:, 3] * (w_orig / w0_)
        labels[:, 4] = labels[:, 4] * (h_orig / h0_)
        if self.pose_data is not None:
            poses = self.pose_data[index].copy()
        if labels.size:
