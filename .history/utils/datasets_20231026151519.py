...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if hyp is not None:
            ...
            if random_percentage < hyp.get('render_phone', ['', 0.0])[1]:
                if len(labels) > 0:
            ...
            #else : if there is cellphone rendering, no phone rendering
            elif hyp is not None and random_percentage < hyp.get('render_ciga', ['', 0.0])[1]:
                if len(labels) > 0:
                    num_of_phone_img = [1,2]
                    num_of_phone = random.randint(num_of_phone_img[0], num_of_phone_img[1])
                    for idx in range(num_of_phone) :
                        phone_img = cv2.imread(os.path.join(hyp.get('render_ciga', None)[0], self.phone_imgs[random.randint(0, len(self.phone_imgs) - 1)]), cv2.IMREAD_UNCHANGED)
                        phone_img = cv2.resize(phone_img, None, fx=0.1+random.random()*0.1, fy=0.1+random.random()*0.1, interpolation=cv2.INTER_LINEAR)
                        if random.random() > 0.5 :
                            phone_img = phone_img[:,::-1,:]
                        if random.random() > 0.75 :
