...
def random_perspective(img, targets=(), segments=(), poses=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    if min_label_size < min_label_size_limit**2:
        natural_min_scale = min_label_size_limit**2 / (img.shape[0]*img.shape[1])
    else:
    ...
    if isinstance(scale, float):
        if natural_min_scale is None:
            ...
            s = random.uniform(natural_min_scale, 1.1 + scale)
    else:
        if natural_min_scale is None:
            s = random.uniform(1 + scale[0], 1.1 + scale[1])
        else:
            s = random.uniform(natural_min_scale, 1.1 + scale[1])
