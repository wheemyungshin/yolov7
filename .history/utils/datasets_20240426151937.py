...
def natural_minmax_crop(image, targets, base_size, min_label_size_limit=None, max_label_size_limit=None):#16, 96
    ...
    if natural_scale is not None:
        ...
        if resized_image.shape[0] > base_h and resized_image.shape[1] > base_w:
            ...
            max_base_size = max(resized_image.shape[1], resized_image.shape[0], base_size)
            max_base_height = int(max_base_size * (height / width))+1
            base_image = np.zeros_like(image)
            base_image = cv2.resize(base_image, (max_base_size, max_base_height))
            base_image = base_image * 0
