...
def random_distortion(im, label, size):
    ...
    xyxys = label[:, 1:].copy()
    xyxys[:, 0] = max(min((label[:, 1] - label[:, 3]*0.5) * w, shape[1]-1), 0)
    xyxys[:, 1] = max(min((label[:, 2] - label[:, 4]*0.5) * h, shape[0]-1), 0)
    xyxys[:, 2] = max(min((label[:, 1] + label[:, 3]*0.5) * w, shape[1]-1), 0)
    xyxys[:, 3] = max(min((label[:, 2] + label[:, 4]*0.5) * h, shape[0]-1), 0)
