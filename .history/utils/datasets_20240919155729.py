...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, ratio_maintain=True, image_weights=False,
        ...
        if len(merge_label) > 0:
            ...
            sampling_ratios_new = []
            labels_new = []
            for x, seg_x in zip(self.label_files, self.img_files, self.shapes, self.segments, self.sampling_ratios, self.labels):
                labels_new.append(x[x[:, 0] < 0])
                segments_new.append(seg_x[x[:, 0] < 0])
                label_files_new.append(lfx)
                img_files_new.append(ifx)
                shapes_new = []
                segments_new = []
                sampling_ratios_new = []
                labels_new = []
