...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, ratio_maintain=True, image_weights=False,
        ...
        if len(merge_label) > 0:
            ...
            sampling_ratios_new = []
            labels_new = []
            for x1, x2, x3, x4, x5, x6 in zip(self.label_files, self.img_files, self.shapes, self.segments, self.sampling_ratios, self.labels):
                label_files_new.append(x1[x6[:, 0] < 0])
                img_files_new.append(x2[x6[:, 0] < 0])
                shapes_new.append(x3[x6[:, 0] < 0])
                segments_new.append(x4[x6[:, 0] < 0])
                sampling_ratios_new.append(x5[x6[:, 0] < 0])
                labels_new.append(x6[x6[:, 0] < 0])
