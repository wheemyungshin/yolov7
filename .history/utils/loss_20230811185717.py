...
from utils.general import bbox_iou, bbox_alpha_iou, box_iou, box_giou, box_diou, box_ciou, xywh2xyxy, crop
...
class ComputeLossSegment:
    ...
    def __call__(self, preds, targets, masks, valid_segment_labels, update_seg_loss=True):  # predictions, targets, model
        p, proto = preds
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                print("t: ", t.shape)
                print("tcls[i]: ", tcls[i])
                print("tcls[i]: ", tcls[i].shape)
                if self.nc > 1:  # cls loss (only if multiple classes)
                    if len(valid_segment_labels) > 0:
                        t_for_obj = torch.clone(t)
                        t_for_obj = t_for_obj[:, :-len(valid_segment_labels)]
                        valid_class_idx = (t_for_obj.sum(dim=1)!=0).bool()
                        t_for_obj = t_for_obj[valid_class_idx]
                        pcls = pcls[:, :-len(valid_segment_labels)]
                        pcls = pcls[valid_class_idx]
                        lcls += self.BCEcls(pcls, t_for_obj)  # BCE
                    else:
                ...
                if len(valid_segment_labels) > 0:
                    iou = bbox_iou(pbox[valid_class_idx].T, tbox[i][valid_class_idx], CIoU=True).squeeze()  # iou(prediction, target)
                    print("pbox.T: ", pbox[valid_class_idx].T.shape)
                    print("tbox[i]: ", tbox[i][valid_class_idx].shape)
                else:
                    iou = bbox_iou(pbox.T, tbox[i], CIoU=True).squeeze()  # iou(prediction, target)
                lbox += (1.0 - iou).mean()  # iou loss
    ...
    def single_mask_loss(self, gt_mask, pred, proto, xyxy, area):
        ...
        print(gt_mask.shape)
        colors = color_list()
        vis_np = np.zeros((gt_mask.shape[1], gt_mask.shape[2], 3))
        for gt_mask_idx, gt_mask_ in enumerate(gt_mask):
            gt_mask_np = gt_mask_.detach().cpu().numpy()
            color = colors[gt_mask_idx % len(colors)]
            vis_np[gt_mask_np!=0] = color
        cv2.imwrite('test_vis_mask_loss/'+str(gt_mask.shape[0])+'.jpg', vis_np)
        '''
