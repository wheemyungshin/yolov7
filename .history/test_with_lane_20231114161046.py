...
def predict_lane(net, input_img, size):
    ...
    all_cls_pred_dilated = np.zeros((1, 1, size[0], size[1])).float()
    torch_resize = transforms.Resize(size)
    for cls_idx in range(5):
        ...
        sig_output_mask = torch.zeros_like(sig_output[0].detach().cpu().float().numpy())
        ...
        for r, c in zip(rows, cols):
            sp = (c-vaf[r, c, 0], r-vaf[r, c, 1]) 
            ep = (c+vaf[r, c, 0], r+vaf[r, c, 1]) 
            cv2.drawline(sig_output_mask, sp, rp, 1, 1)

        sig_output_mask = cv2.resize(sig_output_mask, (size[0], size[1]), interpolation=cv2.INTER_LINEAR)
