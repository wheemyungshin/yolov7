...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            prev_lane_on_lefts = np.array(lane_on_lefts[:int(len(lane_on_lefts)/2)])[prev_lane_on_lefts!=-1]
            temp_lane_on_lefts = np.array(lane_on_lefts[int(len(lane_on_lefts)/2):])
            lrth = 0.48
            if np.sum(prev_lane_on_lefts) > len(prev_lane_on_lefts)*(1-lrth):
                warn_status['on_left'][0] = True
            elif np.sum(prev_lane_on_lefts[prev_lane_on_lefts!=-1]) < len(prev_lane_on_lefts)*lrth:
                warn_status['on_left'][0] = False
            else:
                warn_status['on_left'][0] = None
            if np.sum(temp_lane_on_lefts[temp_lane_on_lefts!=-1]) > len(temp_lane_on_lefts)*(1-lrth):
                warn_status['on_left'][1] = True
            elif np.sum(temp_lane_on_lefts[temp_lane_on_lefts!=-1]) < len(temp_lane_on_lefts)*lrth:
            ...
            if np.sum(prev_lane_going_lefts[prev_lane_going_lefts!=-1]) > len(prev_lane_going_lefts)*(1-lrth):
                warn_status['going_left'][0] = True
            elif np.sum(prev_lane_going_lefts[prev_lane_going_lefts!=-1]) < len(prev_lane_going_lefts)*lrth:
                warn_status['going_left'][0] = False
            else:
                warn_status['going_left'][0] = None
            if np.sum(temp_lane_going_lefts[temp_lane_going_lefts!=-1]) > len(temp_lane_going_lefts)*(1-lrth):
                warn_status['going_left'][1] = True
            elif np.sum(temp_lane_going_lefts[temp_lane_going_lefts!=-1]) < len(temp_lane_going_lefts)*lrth:
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if warn_status['on_left'][0] == False and warn_status['on_left'][0] == True :
