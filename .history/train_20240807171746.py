...
from models.common import Conv, InvertedResidualBlock, DWConvblock
...
def fuse_modules(model):
    for i, m in enumerate(model.modules()):
        ...
        elif type(m) == InvertedResidualBlockReLU:
            for idx in range(len(m.conv)):
                if type(m.conv[idx]) == nn.Conv2d:
                    torch.quantization.fuse_modules(m.conv, [str(idx), str(idx + 1), str(idx + 2)], inplace=True)
        elif type(m) == DWConvblock:
            torch.quantization.fuse_modules(m, [['conv1', 'bn1', 'relu1'], ['conv2', 'bn2', 'relu2']], inplace=True)

    
    '''
    torch.quantization.fuse_modules(model, [['conv', 'bn'],
                                            ['conv1', 'bn1'],
                                            ['conv2', 'bn2'],
                                            ], inplace=True)
    '''
...
def train(hyp, opt, device, tb_writer=None):
    ...
    if opt.seg:
        if len(opt.valid_segment_labels) > 0:
            #nc = nc - len(opt.valid_segment_labels)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        '''
        hyp['degrees'] = ((epoch / epochs))*70
        hyp['hsv_v'] = ((epoch / epochs))*0.5 + 0.1
        hyp['hsv_s'] = ((epoch / epochs))*0.3 + 0.4
        hyp['translate'] = ((epoch / epochs))*0.3
        hyp['scale'] = [-0.55 - ((epoch / epochs))*0.25, -0.45 + ((epoch / epochs))*0.25]
        ...
        for i, (imgs, targets, paths, _, masks) in pbar:  # batch -------------------------------------------------------------
            ...
            '''
            if epoch < 5:
                update_seg_loss = False
            elif epoch > 200:
                update_seg_loss = False
            else:
                update_seg_loss = True
            '''
            ...
            if opt.qat:
                ...
                with amp.autocast(enabled=cuda):
                    pred = model(imgs)  # forward
                    if opt.seg:
                        loss, loss_items = compute_loss_seg(pred, targets.to(device), masks=masks.to(device).float(), valid_segment_labels=valid_segment_labels, imgs=imgs)  # loss scaled by batch_size               
                    elif 'loss_ota' not in hyp or hyp['loss_ota'] == 1:
                        loss, loss_items = compute_loss_ota(pred, targets.to(device), imgs)  # loss scaled by batch_size
                    else:
                        loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size
