...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            path_path = str(path.resolve())
            if torch.sum(correct[:, 0][pred[:, 4] > 0.4]) / torch.sum(pred[:, 4] > 0.4) > 0.3:
                stats.append((correct.cpu(), pred[:, 4].cpu(), pred[:, 5].cpu(), tcls))
            else:
                path_path_nm+=1
                print(path_path)
                print(path_path.replace('/images/', '/labels/').replace('.jpg', '.txt'))
                os.remove(path_path)
                os.remove(path_path.replace('/images/', '/labels/').replace('.jpg', '.txt'))
