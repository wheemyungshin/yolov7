...
OVERALL_HEIGHT_DICT = { # meter
    0 : 2.2, # 승용차, 차량류는 각도 등에 의해 박스 크기는 실제 전고보다 길 것을 반영하여 조금 더 크게 잡는다
...
def calculate_distance(cxywh, crop_window):
    c = cxywh[0]
    x = cxywh[1]
    y = cxywh[2]
    w = cxywh[3]
    ...
    print(crop_window)
    if y + h/2 > crop_window[3]-0.001 and (crop_window[0]+0.05 < x-w/2 and x+w/2 < crop_window[2]-0.05):
        distance = 2
    else:
        pixel_ratio = h
                ...
                if len(det):
                    ...
                    for temp_box_id, (*xyxy, conf, cls) in enumerate(det[:, :6]):
                        ...
                        temp_distance = calculate_distance([int(cls), float(xywh[0]), float(xywh[1]), float(xywh[2]), float(xywh[3])],
                            [crop_x1/img.shape[2], crop_y1/img.shape[1], crop_x2/img.shape[2], crop_y2/img.shape[1]])
