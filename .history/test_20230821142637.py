    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            if opt.seg: #only bs 1 is possible
                ...
                if len(valid_cls_idx) > 0:
                    semantic_gt_mask = torch.zeros((len(valid_cls_idx), masks.shape[1], masks.shape[2]), dtype=torch.float, device=device)
                else:
                    semantic_gt_mask = torch.zeros((nc, masks.shape[1], masks.shape[2]), dtype=torch.float, device=device)
