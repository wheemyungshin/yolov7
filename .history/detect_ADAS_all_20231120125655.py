    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            if lane_med_num_prev > 0 and lane_med_num_temp > 0:
                ...
                if lane_med_sum_prev < int((image_masks.shape[1]/5)*2/5) and int((image_masks.shape[1]/5)*3/5) < lane_med_sum_temp:
                    warn_status['crossline'] = [True, 30]
                elif lane_med_sum_temp < int((image_masks.shape[1]/5)*2/5) and int((image_masks.shape[1]/5)*3/5) < lane_med_sum_prev:
                    warn_status['crossline'] = [True, 30]
                else:
