...
def test(data,
    ...
    dataloader = create_dataloader(data['val'], imgsz, 1, gs, opt, pad=0.5, rect=True,
                                    prefix=colorstr(f"{'val'}: "), valid_idx=valid_idx, load_seg=(opt_task=='seg'))[0]
    ##############################[E. 테스트 데이터 불러오기]############################## 
    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            seen += 1
            if opt_task == 'det' or opt_task == 'cls':
                ...
                #[NOTA]세그멘테이션 결과와 실제 정답 사이의 IoU(겹침정도)를 계산하여 얼마나 유사한 마스크를 만들어냈는지 점수화합니다.
                ##############################[J. Mask IoU 계산]##############################
    ...
    elif opt_task == 'seg': # Print Segmentation Results
...
if __name__ == '__main__':
    ...
    ##############################[B. 테스트 함수 호출]##############################
    if opt.task in ('det', 'seg', 'cls'):  # run normally
        test(opt.data,
            opt.weights,
            opt.img_size,
            opt.conf_thres,
        ...
        print("task is not 'det' or 'seg' or 'cls'.")
    ##############################[B. 테스트 함수 호출]##############################
##############################[A. 시작 부분]##############################