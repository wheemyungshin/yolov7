        ...
        if quadrant == -1:
            ...
            for y_i in range(patch_num[0]):
                for x_i in range(patch_num[1]):
                    ...
                    if x_i != 0:
                        pred_raw = pred_raw[pred_raw[:, 0] > margin/8]
                    if x_i != patch_num[1]-1:
                        pred_raw = pred_raw[pred_raw[:, 2] < patch.shape[3]-margin/8]
                    if y_i != 0:
                        pred_raw = pred_raw[pred_raw[:, 1] > margin/8]
                    if y_i != patch_num[0]-1:
                        pred_raw = pred_raw[pred_raw[:, 3] < patch.shape[2]-margin/8]

                    pred_raw[:, 0] = pred_raw[:, 0] + x1
                    pred_raw[:, 1] = pred_raw[:, 1] + y1
                    pred_raw[:, 2] = pred_raw[:, 2] + x1
                    pred_raw[:, 3] = pred_raw[:, 3] + y1
                    pred_list.append(pred_raw.detach().cpu().numpy())
        else:
            ...
            pred_prev[quadrant] = pred_raw.detach().cpu().numpy()
            for quadrant_idx in range((patch_num[0]*patch_num[1])):
                pred_list.append(pred_prev[quadrant_idx])
                
        pred_concat = pred_list[0]
        for pred_ in pred_list[1:]:
            if pred_ is not None:
                pred_concat = np.concatenate((pred_concat, pred_), axis=0)

        valid_head_list = []
        for xyxy_head in pred_concat[pred_concat[:, -1] == 1]:
            valid_head_list.append(xyxy_head)
        for xyxy_person in pred_concat[pred_concat[:, -1] == 0]:
            xyxy_person_np = xyxy_person
            is_in_head_all = False
            for xyxy_head in pred_concat[pred_concat[:, -1] == 1]:
                xyxy_head_np = xyxy_head
                is_in_head = check_head_in_body(xyxy_head_np[:4], xyxy_person_np[:4], 10)
                if is_in_head:
                    is_in_head_all = True
            if not is_in_head_all:
                valid_head_list.append(np.array([xyxy_person_np[0],xyxy_person_np[1],xyxy_person_np[2],(xyxy_person_np[1]*2 + xyxy_person_np[3])/3,
                        xyxy_person_np[4], xyxy_person_np[5]]))

        ###NMS START###
        valid_head_list = np.array(valid_head_list)
        iou_matrix = _iou(valid_head_list, valid_head_list)
        iou_matrix[iou_matrix==1] = 0
        max_matrix = np.max(iou_matrix, 0)
        is_invalid_head = max_matrix > 0.5
        is_unique = [False]*len(max_matrix)
        for is_unique_idx in np.array(list(np.unique(max_matrix, return_index=True)[1])):
            is_unique[is_unique_idx] = True
        is_invalid_head = is_invalid_head * is_unique
        is_valid_head = np.logical_not(is_invalid_head)
        valid_head_list = valid_head_list[is_valid_head]
        ###NMS END###

        pred_concat = torch.from_numpy(valid_head_list).to(device)
        print(pred_concat)
        print(pred_concat.shape)
        pred = [torch.cat(tuple([pred_concat]), 0)]
