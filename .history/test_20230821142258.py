...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            if opt.seg: #only bs 1 is possible
                pred_mask = torch.flatten(torch.unsqueeze(seg_pred.gt_(0.5).float() , 0), start_dim=1)
                if len(valid_cls_idx) > 0:
                    semantic_gt_mask = torch.zeros((len(valid_cls_idx), masks.shape[0], masks.shape[1]), dtype=torch.long, device=self.device)
                else:
                    semantic_gt_mask = torch.zeros((nc, masks.shape[0], masks.shape[1]), dtype=torch.long, device=self.device)
                
                for t_cls_idx, t_cls in enumerate(targets[:, 1]):
                    semantic_gt_mask[t_cls][((masks[t_cls_idx]).sum(dim=0)!=0).bool()] = valid_idx_i + 1
                    semantic_gt_mask[t_cls] = masks[t_cls_idx]

                gt_mask = torch.flatten(masks.float(), start_dim=1)
                ious = mask_iou(pred_mask, gt_mask)
                print(ious)
                print(ious.shape)
