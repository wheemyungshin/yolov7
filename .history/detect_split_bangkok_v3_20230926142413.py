...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if quadrant == -1:
            ...
            for y_i in range(patch_num[0]):
                for x_i in range(patch_num[1]):
                    ...
                    if x_i != patch_num[1]-1:
                        pred_raw = pred_raw[pred_raw[:, 2] < patch.shape[1]-margin/8]
                    if y_i != 0:
                        pred_raw = pred_raw[pred_raw[:, 1] > margin/8]
                    if y_i != patch_num[0]-1:
                        pred_raw = pred_raw[pred_raw[:, 3] < patch.shape[0]-margin/8]
            ...
            ###remove boxes in borderlines
            if x_i != 0:
                pred_raw = pred_raw[:, pred_raw[0, :, 0]-pred_raw[0, :, 2]/2 > margin/8]
            if x_i != patch_num[1]-1:
                pred_raw = pred_raw[:, pred_raw[0, :, 0]+pred_raw[0, :, 2]/2 < patch.shape[3]-margin/8]
            if y_i != 0:
                pred_raw = pred_raw[:, pred_raw[0, :, 1]-pred_raw[0, :, 3]/2 > margin/8]
            if y_i != patch_num[0]-1:
                pred_raw = pred_raw[:, pred_raw[0, :, 1]+pred_raw[0, :, 3]/2 < patch.shape[2]-margin/8]
