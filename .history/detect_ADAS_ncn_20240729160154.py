                ...
                if len(det):
                    ...
                    for temp_box_id, (*xyxy, conf, cls) in enumerate(det[:, :6]):
                        ...
                        if temp_box_id in valid_idx:
                            ...
                            xyxy_to_vis = xyxy.detach().cpu().float().numpy()
                            xyxy_to_vis = np.array([prev_boxes[prev_box_id][0]*0.34 + xyxy_to_vis[0]*0.66,
                                                    prev_boxes[prev_box_id][1]*0.34 + xyxy_to_vis[1]*0.66,
                                                    prev_boxes[prev_box_id][2]*0.34 + xyxy_to_vis[2]*0.66,
                                                    prev_boxes[prev_box_id][3]*0.34 + xyxy_to_vis[3]*0.66])
                        else:
