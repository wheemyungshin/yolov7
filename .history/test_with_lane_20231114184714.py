...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader)):
        ...
        for si, pred in enumerate(out):
            ...
            if torch.max(ious) < 0.2:
                print(str(path.resolve()))
                print(str(path.resolve()).replace('images', 'segments').replace('.jpg', '.txt'))
                shutil.rmtree(str(path.resolve()))
                shutil.rmtree(str(path.resolve()).replace('images', 'segments').replace('.jpg', '.txt'))
