                ...
                if len(det):
                    ...
                    for temp_box_id, (*xyxy, conf, cls) in enumerate(det[:, :6]):
                        ...
                        if temp_box_id in valid_idx:
                            ...
                            print("prev_boxes_list:", prev_boxes_list)
                            print("prev_box_ids: ", prev_box_ids)
                            xyxy = np.array([
                                prev_weighted_value([prev_boxes_list[-prev_frame_idx][prev_box_ids[-prev_frame_idx]][0] for prev_frame_idx in range(max_prev_frame,0,-1) if len(prev_boxes_list[-prev_frame_idx])], xyxy[0].detach().cpu().float()),
                                prev_weighted_value([prev_boxes_list[-prev_frame_idx][prev_box_ids[-prev_frame_idx]][1] for prev_frame_idx in range(max_prev_frame,0,-1) if len(prev_boxes_list[-prev_frame_idx])], xyxy[1].detach().cpu().float()),
                                prev_weighted_value([prev_boxes_list[-prev_frame_idx][prev_box_ids[-prev_frame_idx]][2] for prev_frame_idx in range(max_prev_frame,0,-1) if len(prev_boxes_list[-prev_frame_idx])], xyxy[2].detach().cpu().float()),
                                prev_weighted_value([prev_boxes_list[-prev_frame_idx][prev_box_ids[-prev_frame_idx]][3] for prev_frame_idx in range(max_prev_frame,0,-1) if len(prev_boxes_list[-prev_frame_idx])], xyxy[3].detach().cpu().float()),
                                ])
                        else:
