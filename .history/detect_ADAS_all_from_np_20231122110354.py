...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            with open(os.path.join(det_source, p.name.split('/')[-1].split('.')[0]+'.txt'), 'r') as f:
                ...
                for xyxy_line_idx, xyxy_line in enumerate(xyxy_lines):
                    if len(xyxy_line.strip().split(' ')) == 5:
                        ...
                        if int(c) == 2:
                            traffic_color = xyxy_lines[xyxy_line_idx].strip()
                        else:
                            traffic_color = 'Nolight'

                        x1_vis = (max(min((float(x)-float(w)/2)*output_width, output_width), 0))
                        y1_vis = (max(min((float(y)-float(h)/2)*output_height, output_height), 0))
                        x2_vis = (max(min((float(x)+float(w)/2)*output_width, output_width), 0))
                        y2_vis = (max(min((float(y)+float(h)/2)*output_height, output_height), 0))
