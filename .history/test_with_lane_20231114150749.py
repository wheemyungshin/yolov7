...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            seg_pred = process_semantic_mask(proto[si], pred[:, 6:], pred[:, :6], img.shape[2:], upsample=False)
            ...
            image_masks = semantic_gt_mask[-2].detach().cpu().numpy().astype(float)#[label_indexing]
            ...
            image_masks = all_cls_pred_dilated[0][0].detach().cpu().numpy().astype(float)#all_cls_pred_dilated[0][0].detach().cpu().numpy().astype(float)#[label_indexing]
            image_masks = cv2.resize(image_masks, (vis_img.shape[1], vis_img.shape[0]), interpolation = cv2.INTER_NEAREST)
