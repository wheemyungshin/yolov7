...
CAMERA_ANGLE_WEIGHT = math.tan((CAMERA_LENS_ANGLE/2/180)*math.pi)*2
...
def calculate_distance(cxywh):
    ...
    benchmark_distance = (OVERALL_HEIGHT_DICT[c]) * (10) / CAMERA_ANGLE_WEIGHT # 화면상 10%를 차지할 때의 거리
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            np.save(os.path.join(save_dir, 'mask_npy', 'lane', p.name.split('.')[0]+'_'+'0'*(6-len(str(frame)))+str(frame)), save_lane_np)
            ...
            for i, det in enumerate(pred_seg):  # detections per image
                if len(det):
                    ...
                    np.save(os.path.join(save_dir, 'mask_npy', 'drivable_area', p.name.split('.')[0]+'_'+'0'*(6-len(str(frame)))+str(frame)), save_road_np)
