                ...
                if len(det):
                    ...
                    for temp_box_id, (*xyxy, conf, cls) in enumerate(det[:, :6]):
                        ...
                        if temp_box_id in valid_idx:
                            ...
                            print(xyxy)
                            xyxy_to_vis = np.array([prev_boxes[prev_box_id][0]*0.34 + xyxy[0].detach().cpu().float().numpy()*0.66,
                                                    prev_boxes[prev_box_id][1]*0.34 + xyxy[1].detach().cpu().float().numpy()*0.66,
                                                    prev_boxes[prev_box_id][2]*0.34 + xyxy[2].detach().cpu().float().numpy()*0.66,
                                                    prev_boxes[prev_box_id][3]*0.34 + xyxy[3].detach().cpu().float().numpy()*0.66])
                        else:
