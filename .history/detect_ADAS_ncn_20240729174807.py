                ...
                if len(det):
                    ...
                    for temp_box_id, (*xyxy, conf, cls) in enumerate(det[:, :6]):
                        ...
                        if temp_box_id in valid_idx:
                            for prev_frame_idx in range(max_prev_frame):
                                prev_box_id = investigate_prev_list(match_box_ids_y_list, temp_box_id, maxiter=prev_frame_idx+1)
                                prev_distance = investigate_prev_list(prev_distances_list, prev_box_id, maxiter=prev_frame_idx+1)
                                temp_distance = round(prev_distance*0.34 + temp_distance*0.66, 3)
