...
distance = str(calculate_distance([int(c), float(x), float(y), float(w), float(h)]))
                            ...
                            cv2.rectangle(draw_img, c1, c2, (0,0,0), -1, cv2.LINE_AA)  # filled
                            cv2.putText(draw_img, print_txt, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
