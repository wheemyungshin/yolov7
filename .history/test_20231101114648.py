    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            if not compute_loss and opt_seg: #only bs 1 is possible
                ...
                image_masks = semantic_gt_mask.detach().cpu().numpy().astype(float)#[label_indexing]
                print("pred: ", image_masks.shape)
                ...
                for image_mask_idx in range(1, nc):
                    vis_mask[image_masks==image_mask_idx] = np.array([50,50,255])
                alpha = 0.5
