...
def detect(save_img=False):    
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if dataset.mode != 'image' and frame % frame_ratio != 0:
            ...
            if np.sum(prev_lane_going_lefts[prev_lane_going_lefts==-1]) > len(prev_lane_going_lefts)*0.5 and \
                np.sum(prev_lane_going_lefts[prev_lane_going_lefts==-1]) > len(prev_lane_going_lefts)*0.5
