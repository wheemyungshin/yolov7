...
def predict_lane(net, input_img, epoch):
    ...
    all_cls_pred = torch.unsqueeze(torch.unsqueeze(torch.zeros_like(input_img[0][0].float()), 0), 0)
    torch_resize = transforms.Resize((input_img.shape[-2], input_img.shape[-1]))
...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            print(pred_masks_per_cls.shape)
            print(all_cls_pred_dilated.shape)
