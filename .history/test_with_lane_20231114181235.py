...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes, masks) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            pred_masks_per_cls = torch.zeros((masks.shape[1], masks.shape[2]), dtype=torch.long, device=device)
            semantic_gt_mask = torch.zeros((masks.shape[1], masks.shape[2]), dtype=torch.long, device=device)
            for t_cls_idx, t_cls in enumerate(targets[targets[:, 0] == si, 1]):
                if int(t_cls) >= 12:
                    pred_masks_per_cls[seg_pred==t_cls+1] = 1
                    semantic_gt_mask[((masks[targets[:, 0] == si][t_cls_idx])!=0).bool()] = 1
