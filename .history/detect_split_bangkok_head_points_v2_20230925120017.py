...
def detect(save_img=False):
    ...
    pred_list = []
    margin = 64
    patch_stride_x = int((imgsz[1]-margin) / list(opt.patch_num)[1])
    patch_stride_y = int((imgsz[0]-margin) / list(opt.patch_num)[0])
    
    if trace:
        if len(opt.img_size)==2:
            model = TracedModel(model, device, (check_img_size(patch_stride_y+margin, 64), check_img_size(patch_stride_x+margin, 64)))
