...
def detect(save_img=False):    
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        txt_path = str(save_dir / 'labels' / save_path.split('_clean')[0]) + ('' if dataset.mode == 'image' else '_'+'0'*(6-len(str(frame)))+str(frame))  # img.txt
            ...
            if os.path.exists(os.path.join(det_source, p.name.split('/')[-1].split('_clean')[0]+'.txt')):
                ...
                with open(os.path.join(det_source, p.name.split('/')[-1].split('_clean')[0]+'.txt'), 'r') as f:
                    ...
                    for xyxy_line_idx, xyxy_line in enumerate(xyxy_lines):
                        if len(xyxy_line.strip().split(' ')) == 5:
                            ...
                            print((float(h)*im0.shape[0])*(float(w)*im0.shape[1]))
                            if (float(w)*im0.shape[1]) > 300:
                                continue

                            if float(y) > 0.8:
                                continue

                            if frame_by_name > 210 and float(x) < 0.5:
                                continue
                ...
                if save_img:
                    ...
                    if opt.save_frame:
                        ...
                        cv2.imwrite(os.path.join(save_dir, 'images', p.name.split('_clean')[0])+'.jpg', clean_im0)
