...
import models
from models.experimental import attempt_load, attempt_load_v2, End2End, End2End_seg
...
if __name__ == '__main__':
    ...
    if opt.cfg and opt.nc:
        ...
        ckpt = Model(opt.cfg, ch=3, nc=opt.nc, qat=opt.qat).to(map_location)  # create#, nm=nm).to(device)  # create
        model = attempt_load_v2(opt.weights, opt.cfg, opt.nc, opt.qat, map_location=device)  # load FP32 model
    else:
