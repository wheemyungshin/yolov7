...
class ISegment(nn.Module):
    ...
    def forward(self, x):
        ...
        return (None, proto_output) if self.training else (x[0], proto_output) if self.export else (x[0], (x[1], proto_output))
...
class ISegment(IDetect):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), nm=None):
        ...
        #self.im = nn.ModuleList(ImplicitM(self.no * self.na) for _ in ch)
        #self.protos = nn.ModuleList(Proto(x, self.npr, self.nm) for x in ch)  # protos
        self.proto = Proto(ch[-1], self.npr, self.nm)  # protos
        self.detect = IDetect.forward
    ...
    def forward(self, x):
        x_origin = x.copy()
        x = self.detect(self, x)
        p_input = x_origin[-1] # conv
        proto_output = self.proto(p_input)
