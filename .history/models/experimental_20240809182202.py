...
from utils.general import process_semantic_mask
...
class ONNX_ORT(nn.Module):
    ...
    def __init__(self, max_obj=100, iou_thres=0.45, score_thres=0.25, max_wh=640, device=None, n_classes=80, agnostic_nms=False):
        super().__init__()
...
class End2End_seg(nn.Module):
    ...
    def forward(self, x):
        ...
        proto = out[1][0]
        det = pred[0]
        mask = process_semantic_mask(proto, det[:, 6:], det[:, :6], (proto.shape[1]*8, proto.shape[2]*8), upsample=True)
        return mask
...
def attempt_load_v2(weights, map_location=None):
    # Loads an ensemble of models weights=[a,b,c] or a single model weights=[a] or weights=a
    model = Ensemble()
    for w in weights if isinstance(weights, list) else [weights]:
        attempt_download(w)
...
def attempt_load_v2(weights, cfg, nc, map_location=None):
    ens_model = Ensemble()
    ...
    if cfg and nc:
        ...
        model = Model(cfg, ch=3, nc=nc, qat=opt.qat).to(device)  # create#, nm=nm).to(device)  # create
        ...
        for name, m in model.named_modules():
            if type(m) in [nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU]:
                ...
                model.model[int(layer_id)]._non_persistent_buffers_set = set()  # pytorch 1.6.0 compatibility