...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None, nm=None, qat=False):  # model, input channels, number of classes
    ...
    def forward_once(self, x, profile=False, get_feature=False):
        ...
        if self.qat:
            print("2QATQATQATQATQATQATQATQATQATQATQATQATQATQATQATQAT")
            x = self.quant(x)
        ...
        if self.qat:        
            print("1QATQATQATQATQATQATQATQATQATQATQATQATQATQATQATQAT")    
            x = self.dequant(x)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
            ...
            if m in [DownC, SPPCSPC, GhostSPPCSPC, 
                ...
                n = 1
        elif m in [Shuffle_Like_Block, Shuffle_Block, DWConvblock, DWConvblocknoBN, StemBlock, InvertedResidualBlock, InvertedResidualBlockReLU, InvertedResidualBlockReLUnoBN]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)
            args = [c1, c2, *args[1:]]
