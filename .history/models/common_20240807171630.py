...
def channel_shuffle_like(x, groups):
    batchsize, num_channels, height, width = x.data.size()
    channels_per_group = num_channels // groups
    unit = int(num_channels * 0.25)
    x_shuffle = torch.cat([x[:,unit*3:unit*4,:,:], x[:,0:unit*3,:,:]], dim=1) 
    return x_shuffle
...
class InvertedResidualBlockReLUnoBN(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio=2):
        super(InvertedResidualBlockReLUnoBN, self).__init__()
        self.stride = stride
        assert stride in [1, 2]
...
class Shuffle_Like_Block(nn.Module):
    def __init__(self, inp, oup, stride):
        super(Shuffle_Like_Block, self).__init__()
    ...
    def forward(self, x):
        ...
        out = channel_shuffle_like(out, 2)
...
class PSA(nn.Module):
    ...
    def __init__(self, c1, c2, e=0.5):
        ...
        self.attn = Attentionv10(self.c, attn_ratio=0.5, num_heads=self.c // 64)
