...
def attempt_load_v2(weights, cfg, nc, qat, map_location=None):
    model = Ensemble()
    
    if cfg and nc:
        ckpt_weights = torch.load(weights, map_location=map_location)  # load checkpoint
        ...
        ckpt = Model(cfg, ch=3, nc=nc, qat=qat).to(map_location)  # create#, nm=nm).to(device)  # create
