...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        pred_lane = non_max_suppression(pred_lane, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms)
        pred_lane[0][:, 0] = pred_lane[0][:, 0] + crop_x1
        pred_lane[0][:, 2] = pred_lane[0][:, 2] + crop_x1
        
        left_xyxy = [im0s.shape[1]*0.1, im0s.shape[0]*0.65, im0s.shape[1]*0.4, im0s.shape[0]-1]
        right_xyxy = [im0s.shape[1]*0.6, im0s.shape[0]*0.65, im0s.shape[1]*0.9, im0s.shape[0]-1]
        ...
        print(draw_polygons.shape)
        fill_image = im0.copy()
        fill_image = cv2.fillPoly(fill_image, pts=draw_polygons, color=(0,255,0))
        alpha = 0.5
        im0 = cv2.addWeighted(fill_image, alpha, im0, 1 - alpha, 0)
        cv2.line(im0, (int(right_xyxy[2]),int(right_xyxy[1])), (int(right_xyxy[0]),int(right_xyxy[3])), [255, 0, 255], 3)
        cv2.line(im0, (int(left_xyxy[0]),int(left_xyxy[1])), (int(left_xyxy[2]),int(left_xyxy[3])), [255, 0, 255], 3)
        
        lines_rightxyxy_to_draw = [str(int(right_xyxy[0])), str(int(right_xyxy[1])), str(int(right_xyxy[2])), str(int(right_xyxy[3]))]
        lines_leftxyxy_to_draw = [str(int(right_xyxy[0])), str(int(right_xyxy[1])), str(int(right_xyxy[2])), str(int(right_xyxy[3]))]
